...
$(`<a href="#" class="btn btn-primary" style="padding: 1px 5px 3px;margin-top: -35px;width: 22%;float: right;position: relative;margin-right: 1px;font-family: 'jazeera',FontAwesome;font-size: 13px!important;background-color: #ffffff!important;color: #337ab7;">القوانين</a>
<a href="#" class="btn btn-primary" style="padding: 1px 5px 3px;width: 22%;margin-top: -8px;float: right;position: relative;margin-right: 1px;font-family: 'jazeera',FontAwesome;font-size: 13px!important;background-color: #ffffff!important;color: #337ab7;">اختصارات</a>
`).appendTo('div#l1');
$(`<a href="#" class="btn btn-primary" style="padding: 1px 5px 3px;margin-top: -26px;width: 22%;float: right;position: relative;margin-right: 1px;font-family: 'jazeera',FontAwesome;font-size: 13px!important;background-color: #ffffff!important;color: #337ab7;">ق.السوابر</a>
<a href="#" class="btn btn-primary" style="padding: 1px 5px 3px;width: 22%;float: right;position: relative;margin-right: 1px;font-family: 'jazeera',FontAwesome;font-size: 13px!important;background-color: #ffffff!important;color: #337ab7;">اشتراكات</a>
`).appendTo('div#l3');
$("div#tlogins button.btn.btn-primary").click(function() {
	var myVar = setInterval(function() {
		var usmsgw = $(".pmsgc").length;
...
if (xharf === 12 || xharf === 8) {
	$(`<img style="width:100%;margin-top: 1px;float: left;margin-bottom: 4px;" src="https://galychat.com/sico/z1crbngl1510.jpg" class="fr">`).insertBefore('.nav-tabs');
...
var socket, users = [], rooms = [], myid = null, myroom = null, nopm = !0, nonot = !0, pickedfile = null, power = {}, powers = [], emos = [], dro3 = [], token = '', rbans = [], blocked = [];
function sendbc(C) {
    C ? (pickedfile = null,
...
function refr() {
    ...
    return 0 == C.indexOf('http://' + location.hostname) ? '' : (-1 != C.indexOf('://') && (C = C.replace(/(.*?)\:\/\//g, '').split('/')[0]),
    C)
}
function checkupdate() {
    needUpdate && (updateusers(),
    updaterooms(),
    needUpdate = !1),
    setTimeout(checkupdate, 2e3)
}
function load() {
    if (isIphone = /ipad|iphone|ipod/i.test(navigator.userAgent.toLowerCase()),
    'undefined' == typeof $ || 'undefined' == typeof io)
        return void close(5e3);
    if (null == $('').tab)
        return void close(5e3);
    isIphone && ($('img[data-toggle="popover"]').removeClass('nosel'),
    fxi()),
    checkupdate(),
    ...
    'yes' == getv('isl') && $('.nav-tabs a[href="#l2"]').tab('show'),
    uhtml = $('#uhtml').html(),
    rhtml = $('#rhtml').html(),
    $('.ae').click(function() {
        $('.phide').click()
    ...
    processq(),
    newsock(),
    '' == getv('refr') && setv('refr', refr() || '*'),
    '' == getv('r') && setv('r', getUrlParameter('r') || '*'),
...
function newsock() {
    ...
    socket.on('connect', function() {
        ...
        pending = !1),
        null != getUrlParameter('enter') && ($('#u1').val(hash([new Date().getTime()], 256) + '_\u0632\u0627\u0626\u0631'),
        login(1))
    }),
    socket.on('re', function(C) {
        !0 == C.ok ? (!1 == pending,
        pdata.forEach(function(S) {
            socket.emit('msg', S)
        }),
        pdata = []) : close()
    }),
    ...
    socket.on('disconnect', function() {
        return null != myid && !1 == pending ? (pending = !0,
        void setTimeout(newsock, 12000)) : void (lstat('danger', '\u063A\u064A\u0631 \u0645\u062A\u0635\u0644'),
        close())
    }),
...
function processq() {
    for (var C = 0, S; C < onq.length && 20 > C; C++)
        S = onq[0],
        onq.splice(0, 1),
        ondata(S.cmd, S.data);
    setTimeout(function() {
    ...
    $.get('getonline', function(C) {
        ...
        $.each(S.online, function(R, D) {
            if (!0 != D.s) {
                ...
                M.find('.ustat').remove(),
                '--' == D.co || null == D.co || 'A1' == D.co || 'A2' == D.co || 'EU' == D.co ? M.find('.co').remove() : M.find('.co').attr('src', 'https://iqr30.com/sico/z1cki553su10.gif');
                var A = getico(D);
...
function roomChanged(C) {
    ...
    if ($('.bord').removeClass('bord'),
    null != S) {
        $('.inr,.ninr,.rout').show(),
        0 == $('#room.active').length && !0 == C && $('[data-target=\'#room\']').trigger('click'),
        !0 == C && $('[data-target=\'#room\']').show(),
...
function ondata(C, S) {
    try {
        ...
        case 'emos':
            ...
            break;
        case 'login':
            switch ($('#tlogins button').removeAttr('disabled'),
            S.msg) {
            case 'ok':
                mylat = S.lat,
                myid = S.id,
        ...
        case 'powers':
            powers = S;
            for (var E = 0, P; E < powers.length; E++)
                P = powers[E].name,
                '' == P && (P = '_'),
                powers[P] = powers[E];
            var R = getuser(myid);
            null != R && (power = getpower(R.power || ''),
        ...
        case 'power':
            ...
            break;
        case 'not':
            if (null != S.user && 1 != S.force && !0 == nonot)
                return void send('nonot', {
                    id: S.user
                });
        ...
        case 'bc':
            AddMsg('.d2bc', S),
            !1 != $('.dpnl').is(':visible') && $('#wall').hasClass('active') || (bcc++,
        ...
        case 'ops':
            ...
            break;
        case 'pm':
            if (ismuted(getuser(S.uid)))
                return;
            if (1 != S.force && !0 == nopm && 0 == $('#c' + S.pm).length)
                return void send('nopm', {
                    id: S.uid
                });
            openw(S.pm, !1),
            AddMsg('#d2' + S.pm, S),
...
function updateu(C, S) {
    ...
    if (null != E) {
        ...
        E.s && (R = 'imgs/s4.png?2'),
        E.id == myid && ($('.spic').css('background-image', 'url("' + E.pic + '")'),
        $('.stopic').val(gettext($('<div>' + E.topic + '</div>'))),
        $('.smsg').val(gettext($('<div>' + E.msg + '</div>'))),
        $('.scolor').val(E.ucol).css('background-color', E.ucol).trigger('change'),
        $('.mcolor').val(E.mcol || '#000').css('background-color', E.mcol || '#000'),
        $('.sbg').val(E.bg).css('background-color', E.bg)),
        '' == E.msg && (E.msg = '..');
        var D = $('.uid' + C);
        if (D.find('.ustat').attr('src', R),
        '--' == E.co || null == E.co || 'A1' == E.co || 'A2' == E.co || 'EU' == E.co ? D.find('.co').remove() : D.find('.co').attr('src', 'flag/' + E.co.toLowerCase() + '.gif'),
        ismuted(E) ? D.find('.muted').toggleClass('fa-ban', !0) : D.find('.muted').toggleClass('fa-ban', !1),
        D.attr('v', getpower(E.power).rank),
        '' == P ? D.find('.u-ico').removeAttr('src') : D.find('.u-ico').attr('src', P),
        D.find('.u-topic').html(E.topic).css({
            'background-color': E.bg,
            color: E.ucol
        }),
        D.find('.u-msg').html(E.msg),
...
var needUpdate = !1
  , lastus = '';
function usearch() {
    $('#usearch').val() != lastus && (lastus = $('#usearch').val(),
    '' == lastus ? $('#usearch').addClass('bg') : $('#usearch').removeClass('bg'),
    $('#users .uzr').css('display', ''),
    $.each($.grep(users, function(C) {
...
function star(C, S) {
    ...
    switch (0 == E.length && (E = C.parent().find('.fa-star')),
    !0) {
    case 5e3 <= S:
...
function sendpm(C) {
    if (ismuted(getuser(C.data.uid)))
        return void alert('\u0627\u0644\u063A\u064A \u0627\u0644\u062A\u062C\u0627\u0647\u0644\n\u062D\u062A\u0649 \u062A\u062D\u062C\u064A \u0648\u064A\u0627 \u0647\u064A\u0647 \u0645\u0627\u0644 \u062C\u0627\u0642\u0627\u062A');
...
function getpower(C) {
    ...
    for (var E = 0; E < powers.length; E++)
        if (powers[E].name == C)
            return powers[E];
    for (var P = JSON.parse(JSON.stringify(powers[0])), R = Object.keys(P), E = 0; E < R.length; E++)
        switch (!0) {
...
function rjoin(C) {
    var S = '';
    getroom(C).needpass && (S = prompt('\u0644\u062A\u062D\u0627\u0648\u0644 \u0648\u0644\u0627 \u062A\u062A\u0639\u0628 \u0627\u0644\u0628\u0627\u0633\u0648\u0648\u0631\u062F \u0627\u0635\u0639\u0628 \u0645\u0646 \u0628\u0627\u0633\u0648\u0648\u0631\u062F \u0631\u0627\u0648\u062A\u0631\u0643\u0645', ''),
    '' == S) || send('rjoin', {
        id: C,
        pwd: S
    })
...
function AddMsg(C, S) {
    ...
    if (1 < R.length && '#d2' != C && (S.msg = S.msg.replace(R[1], '<button onclick=\'ytube("https://www.youtube.com/embed/' + R[0] + '",this);\' style=\'font-size:40px!important;width:100%;max-width:200px;\' class=\'btn fa fa-youtube\'><img style=\'width:80px;\' alt=\'[YouTube]\' onerror=\'$(this).parent().remove();\' src=\'https://img.youtube.com/vi/' + R[0] + '/0.jpg\' ></button>')),
    E.find('.u-msg').html(S.msg + '&nbsp;').css('color', S.mcol),
    null != S.class && E.addClass(S.class),
    E.addClass('mm'),
    null != P) {
        var D = getico(P);
    ...
    var M = '.d2bc' == C;
    return null != S.bid && (E.addClass('bid' + S.bid),
    (power.delbc || S.lid == getuser(myid).lid) && E.append('<a onclick="send(\'delbc\',{bid:\'' + S.bid + '\'})" style="margin-top:-20px;padding:4px;" class="btn minix btn-primary fa fa-times fr">&nbsp;</a>'),
    E.append('<a onclick="send(\'likebc\',{bid:\'' + S.bid + '\'})" style="margin-top:-20px;padding:4px;" class="btn minix btn-danger fa fa-heart fr">&nbsp;</a>')),
    !0 == M ? E.prependTo($(C)) : E.appendTo($(C)),
    ...
    !0 == M ? (100 <= $(C).find('.mm').length && $(C + ' .mm').last().remove(),
    $(C).scrollTop(0)) : (30 <= $(C).find('.mm').length && $(C + ' .mm').first().remove(),
...
function close(C) {
    ...
    setTimeout('location.reload();', C || 4e3),
    lstat('info', '\u0627\u0635\u0628\u0631'))
}
...
function unmute(C) {
    for (var S = 0, E; S < blocked.length; S++)
        E = blocked[S],
        (E.lid == C.lid || E.username == C.username) && (blocked.splice(S, 1),
        updateu(C.id));
    saveblocked()
}
function muteit(C) {
    if (C.id != myid) {
        for (var S = 0, E; S < blocked.length; S++)
            if (E = blocked[S],
            E.lid == C.lid || E.username == C.username)
                return;
        blocked.push({
            lid: C.lid,
            topic: C.topic,
...
function upro(C) {
    ...
    if (null != E && !(E.s && getpower(E.power).rank > power.rank)) {
        ...
        var D = getico(E)
          , M = '\u0628\u062F\u0648\u0646 \u063A\u0631\u0641\u0647'
          , A = getroom(E.roomid);
        if (!0 == power.unick || !0 == power.mynick && C == myid ? ($('.u-topic').val(E.topic),
        P.find('.nickbox').show(),
        P.find('.u-nickc').off().click(function() {
            send('unick', {
                id: C,
        ...
        power.setpower) {
            ...
            P.find('.powerbox').hide();
        null == A ? P.find('.u-room').hide() : (null != A.ops && (-1 != A.ops.indexOf(getuser(myid).lid) || A.owner == getuser(myid).lid || power.roomowner) && (S = !0),
        M = '<div class="fl btn btn-primary dots roomh border" style="padding:0px 5px;max-width:180px;" onclick="rjoin(\'' + A.id + '\')"><img style="max-width:24px;" src=\'' + A.pic + '\'>' + A.topic + '</div>',
        P.find('.u-room').html(M),
        P.find('.u-room').show()),
        S ? P.find('.urkick,.umod').show() : P.find('.urkick,.umod').hide(),
        ismuted(E) ? (P.find('.umute').hide(),
        P.find('.uunmute').show()) : (P.find('.umute').show(),
        P.find('.uunmute').hide()),
        P.find('.ureport').hide(),
        !0 == power.history ? P.find('.uh').show() : P.find('.uh').hide(),
        1 > power.kick ? (P.find('.ukick').hide(),
        P.find('.udelpic').hide()) : (P.find('.ukick').show(),
        P.find('.udelpic').show()),
        ...
        P.find('.unot').css('background-color', '').off().click(function() {
            ...
            null == F || '' == F || ($(this).css('background-color', 'indianred'),
            send('action', {
                cmd: 'not',
                id: C,
                msg: F
...
function getUrlParameter(C) {
    for (var S = window.location.search.substring(1), E = S.split('&'), P = 0, R; P < E.length; P++)
        if (R = E[P].split('='),
        R[0] == C)
            return ('' + decodeURIComponent(R[1])).split('<').join('&#x3C;')
}
function mkr() {
    $('#ops').children().remove();
    var C = $('#mkr');
...
function updaterooms() {
    if (!1 != needUpdate) {
        var C = getuser(myid);
        null == C || ($('.brooms').text(rooms.length),
        $.each(rooms, function(S, E) {
            var P = $('.r' + E.id);
...
function hash(C, S) {
    ...
    A = 461845907,
    B = 0; B < P; )
        I = 255 & C.charCodeAt(B) | (255 & C.charCodeAt(++B)) << 8 | (255 & C.charCodeAt(++B)) << 36 | (255 & C.charCodeAt(++B)) << 24,
        ++B,
...
function getfp() {
    try {
        ...
        var N = getv('cc') || ''
          , U = getv('fp1') || ''
          , F = getv('refr') || '';
        return '' == U && (U = (S.getOS().replace('Windows', 'Win') + '.' + S.getOSVersion() + '.' + S.getBrowser()).split(' ').join('-').split('_').join('-') + D,
        setv('fp1', U)),
        '' == N && (N = ccode(),
        setv('cc', N)),
...
function openw(C, S) {
    ...
    if (null != E) {
        ...
        S ? ($('.phide').trigger('click'),
        $('.w' + C).css('display', '').addClass('active').show(),
        $('.pn2').hide(),
        setTimeout(function() {
            fixSize(),
            $('.w' + C).find('.d2').scrollTop($('.w' + C).find('.d2')[0].scrollHeight)
        }, 100),
        $('.dpnl').hide()) : 'none' == $('.w' + C).css('display') && $('#c' + C).addClass('unread'),
        msgs()
    }
}
...
function msgs() {
    ...
    0 == C ? ($('.chats').find('.badge').text(''),
    hl($('.chats'), 'primary')) : ($('.chats').find('.badge').text(C),
    hl($('.chats'), 'warning'))
}
var uhd = '*';
function uhead() {
...
(function(C) {
    ...
    C.fn.pop = function(S) {
        if (this.hasClass('pop'))
            return this.find('.popbody').children(0).pop(S);
        switch (S) {
        case 'show':
    ...
    document.styleSheets)
        for (var E = 0; E < document.styleSheets.length; E++) {
            var P = document.styleSheets[E]
              ...
              , D = !1;
            do {
                if (D = P.cssRules ? P.cssRules[R] : P.rules[R],
                D && D.selectorText == C)
                    return 'delete' == S ? (P.cssRules ? P.deleteRule(R) : P.removeRule(R),
                    !0) : D;
                R++
            } while (D)
        }
...
function getCookie(C) {
    for (var S = C + '=', E = document.cookie.split(';'), P = 0, R; P < E.length; P++) {
        for (R = E[P]; ' ' == R.charAt(0); )
            R = R.substring(1);
        if (-1 != R.indexOf(S))
            return decode(R.substring(S.length, R.length))
    }
    return ''
}
cmsg = null;
...
(function(C) {
    ...
    return function P(R, D, M) {
        ...
        for (var I = 'function' == typeof require && require, B = 0; B < M.length; B++)
            A(M[B]);
        return A
    }({
        1: [function(P, R) {
            ...
            M.writeCandidate = function(A) {
                ...
                return I.push('typ'),
                I.push(B),
                'host' !== B && A.relatedAddress && A.relatedPort && (I.push('raddr'),
                I.push(A.relatedAddress),
                I.push('rport'),
                I.push(A.relatedPort)),
                A.tcpType && 'tcp' === A.protocol.toLowerCase() && (I.push('tcptype'),
                I.push(A.tcpType)),
                'candidate:' + I.join(' ')
            }
            ,
            ...
            M.writeRtpMap = function(A) {
                var I = A.payloadType;
                return void 0 !== A.preferredPayloadType && (I = A.preferredPayloadType),
                'a=rtpmap:' + I + ' ' + A.name + '/' + A.clockRate + (1 === A.numChannels ? '' : '/' + A.numChannels) + '\r\n'
            }
            ,
            ...
            M.parseFmtp = function(A) {
                for (var I = {}, N = A.substr(A.indexOf(' ') + 1).split(';'), U = 0, B; U < N.length; U++)
                    B = N[U].trim().split('='),
                    I[B[0].trim()] = B[1];
                return I
            }
            ...
            M.writeFmtp = function(A) {
                var I = ''
                  , B = A.payloadType;
                if (void 0 !== A.preferredPayloadType && (B = A.preferredPayloadType),
            ...
            M.parseSsrcMedia = function(A) {
                var I = A.indexOf(' ')
                  ...
                  , N = A.indexOf(':', I);
                return -1 < N ? (B.attribute = A.substr(I + 1, N - I - 1),
                B.value = A.substr(N + 1)) : B.attribute = A.substr(I + 1),
                B
            }
            ,
            M.getMid = function(A) {
            ...
            M.parseRtpParameters = function(A) {
                for (var I = {
                    ...
                    if (G) {
                        ...
                        switch (z.parameters = V.length ? M.parseFmtp(V[0]) : {},
                        z.rtcpFeedback = M.matchPrefix(A, 'a=rtcp-fb:' + F + ' ').map(M.parseRtcpFb),
                        I.codecs.push(z),
                        z.name.toUpperCase()) {
                        case 'RED':
                        case 'ULPFEC':
            ...
            M.parseRtpEncodingParameters = function(A) {
                var I = [], B = M.parseRtpParameters(A), N = -1 !== B.fecMechanisms.indexOf('RED'), U = -1 !== B.fecMechanisms.indexOf('ULPFEC'), F = M.matchPrefix(A, 'a=ssrc:').map(function(H) {
                    return M.parseSsrcMedia(H)
                }).filter(function(H) {
                    return 'cname' === H.attribute
                }), G = 0 < F.length && F[0].ssrc, V = M.matchPrefix(A, 'a=ssrc-group:FID').map(function(H) {
                    var J = H.split(' ');
                    return J.shift(),
                    J.map(function(W) {
                ...
                B.codecs.forEach(function(H) {
                    if ('RTX' === H.name.toUpperCase() && H.parameters.apt) {
                        ...
                        I.push(J),
                        N && (J = JSON.parse(JSON.stringify(J)),
                        J.fec = {
                            ssrc: z,
                ...
                return q.length && (0 === q[0].indexOf('b=TIAS:') ? q = parseInt(q[0].substr(7), 10) : 0 === q[0].indexOf('b=AS:') && (q = parseInt(q[0].substr(5), 10)),
            ...
            M.writeMediaSection = function(A, I, B, N) {
                ...
                if (U += M.writeIceParameters(A.iceGatherer.getLocalParameters()),
                U += M.writeDtlsParameters(A.dtlsTransport.getLocalParameters(), 'offer' === B ? 'actpass' : 'active'),
                U += 'a=mid:' + A.mid + '\r\n',
                ...
                A.rtpSender) {
                    ...
                    A.sendEncodingParameters[0].rtx && (U += 'a=ssrc:' + A.sendEncodingParameters[0].rtx.ssrc + ' ' + F,
                    U += 'a=ssrc-group:FID ' + A.sendEncodingParameters[0].ssrc + ' ' + A.sendEncodingParameters[0].rtx.ssrc + '\r\n')
                }
            ...
            M.getDirection = function(A, I) {
                for (var B = M.splitLines(A), N = 0; N < B.length; N++)
                    switch (B[N]) {
                    case 'a=sendrecv':
                    case 'a=sendonly':
                    case 'a=recvonly':
            ...
            R.exports = function(M) {
                ...
                case 'chrome':
                    if (!F || !F.shimPeerConnection)
                        return B('Chrome shim is not included in this adapter release.'),
                        U;
                    ...
                    break;
                case 'firefox':
                    if (!z || !z.shimPeerConnection)
                        return B('Firefox shim is not included in this adapter release.'),
                        U;
                ...
                case 'edge':
                    if (!G || !G.shimPeerConnection)
                        return B('MS edge shim is not included in this adapter release.'),
                        U;
                ...
                case 'safari':
                    if (!V)
                        return B('Safari shim is not included in this adapter release.'),
                        U;
              ...
              , I = {
                ...
                shimSourceObject: function(B) {
                    ...
                    'object' == typeof B && B.HTMLMediaElement && !('srcObject'in B.HTMLMediaElement.prototype) && Object.defineProperty(B.HTMLMediaElement.prototype, 'srcObject', {
                        ...
                        set: function(U) {
                            ...
                            U ? void (this.src = N.createObjectURL(U),
                            U.addEventListener('addtrack', function() {
                ...
                shimPeerConnection: function(B) {
                        ...
                        B.RTCPeerConnection = function(z, V) {
                            if (z && z.iceServers) {
                                for (var q = [], H = 0, J; H < z.iceServers.length; H++)
                                    J = z.iceServers[H],
                                    !J.hasOwnProperty('urls') && J.hasOwnProperty('url') ? (console.warn('RTCIceServer.url is deprecated! Use urls instead.'),
                                    J = JSON.parse(JSON.stringify(J)),
                                    J.urls = J.url,
                                    q.push(J)) : q.push(z.iceServers[H]);
                                z.iceServers = q
                            }
                            return new U(z,V)
                        }
                    ...
                    B.RTCPeerConnection.prototype.getStats = function(z, V, q) {
                        var H = this
                          , J = arguments;
                        if (0 < arguments.length && 'function' == typeof z)
                            return F.apply(this, arguments);
                        if (0 === F.length && (0 === arguments.length || 'function' != typeof arguments[0]))
                            return F.apply(this, []);
                        var W = function(K) {
                            var Z = {}
                              , Q = K.result();
                    ...
                    B.RTCPeerConnection.prototype.addIceCandidate = function() {
                        return arguments[0] ? G.apply(this, arguments) : (arguments[1] && arguments[1].apply(null),
                        Promise.resolve())
                    }
                }
            };
        ...
        5: [function(P, R) {
            ...
            R.exports = function(I) {
                var B = M.detectBrowser(I)
                  , N = I && I.navigator
                  , U = function(H) {
                    if ('object' != typeof H || H.mandatory || H.optional)
                        return H;
                    var J = {};
                    return Object.keys(H).forEach(function(W) {
                        if ('require' !== W && 'advanced' !== W && 'mediaSource' !== W) {
                            ...
                            if (void 0 !== X.ideal) {
                                ...
                                var K = {};
                                'number' == typeof X.ideal ? (K[Y('min', W)] = X.ideal,
                                J.optional.push(K),
                                K = {},
                                K[Y('max', W)] = X.ideal,
                                J.optional.push(K)) : (K[Y('', W)] = X.ideal,
                                J.optional.push(K))
                            }
                            void 0 !== X.exact && 'number' != typeof X.exact ? (J.mandatory = J.mandatory || {},
                            J.mandatory[Y('', W)] = X.exact) : ['min', 'max'].forEach(function(Z) {
                                void 0 !== X[Z] && (J.mandatory = J.mandatory || {},
                                J.mandatory[Y(Z, W)] = X[Z])
                            })
                        }
                    }),
                  ...
                  , F = function(H, J) {
                    if (H = JSON.parse(JSON.stringify(H)),
                    H && 'object' == typeof H.audio) {
                        var W = function(Z, Q, ne) {
                            Q in Z && !(ne in Z) && (Z[ne] = Z[Q],
                            delete Z[Q])
                        };
                    ...
                    if (H && 'object' == typeof H.video) {
                        ...
                        if (X && ('user' === X.exact || 'environment' === X.exact || 'user' === X.ideal || 'environment' === X.ideal) && !(N.mediaDevices.getSupportedConstraints && N.mediaDevices.getSupportedConstraints().facingMode && !Y)) {
                            ...
                            var K;
                            if ('environment' === X.exact || 'environment' === X.ideal ? K = ['back', 'rear'] : ('user' === X.exact || 'user' === X.ideal) && (K = ['front']),
                            K)
                                return N.mediaDevices.enumerateDevices().then(function(Z) {
                    ...
                    N.mediaDevices.getUserMedia = function(H) {
                        return F(H, function(J) {
                            return q(J).then(function(W) {
                                if (J.audio && !W.getAudioTracks().length || J.video && !W.getVideoTracks().length)
                                    throw W.getTracks().forEach(function(X) {
                                        X.stop()
                                    }),
        ...
        8: [function(P, R) {
            ...
            function M(U) {
                ...
                for (U = []; F.length || G.length; )
                    F.length && U.push(F.shift()),
            ...
            function I(U, F) {
                var G = {
                  ...
                  , z = function(q, H) {
                    ...
                    for (var J = 0; J < H.length; J++)
                        if (H[J].payloadType === q || H[J].preferredPayloadType === q)
                            return H[J]
                }
                  , V = function(q, H, J, W) {
                ...
                return U.codecs.forEach(function(q) {
                    for (var H = 0, J; H < F.codecs.length; H++)
                        if (J = F.codecs[H],
                        q.name.toLowerCase() === J.name.toLowerCase() && q.clockRate === J.clockRate) {
                            if ('rtx' === q.name.toLowerCase() && q.parameters && J.parameters.apt && !V(q, J, U.codecs, F.codecs))
                                continue;
                            J = JSON.parse(JSON.stringify(J)),
                            ...
                            J.rtcpFeedback = J.rtcpFeedback.filter(function(W) {
                                for (var X = 0; X < q.rtcpFeedback.length; X++)
                                    if (q.rtcpFeedback[X].type === W.type && q.rtcpFeedback[X].parameter === W.parameter)
                                        return !0;
                                return !1
                            });
                            break
            ...
            R.exports = function(U, F) {
                var G = function(z) {
                    ...
                    z && z.iceTransportPolicy)
                        switch (z.iceTransportPolicy) {
                        case 'all':
                        case 'relay':
                ...
                G.prototype._emitBufferedCandidates = function() {
                    ...
                    this._localIceCandidatesBuffer.forEach(function(q) {
                        ...
                        if (H)
                            ...
                            V[q.candidate.sdpMLineIndex + 1] += 'a=' + q.candidate.candidate + '\r\n';
                        if (z.localDescription.sdp = V.join(''),
                        z.dispatchEvent(q),
                        null !== z.onicecandidate && z.onicecandidate(q),
                        !q.candidate && 'complete' !== z.iceGatheringState) {
                            var W = z.transceivers.every(function(X) {
                                return X.iceGatherer && 'completed' === X.iceGatherer.state
                            });
                            W && 'complete' !== z.iceGatheringStateChange && (z.iceGatheringState = 'complete',
                            z._emitGatheringStateChange())
                        }
                    }),
                    this._localIceCandidatesBuffer = []
                }
                ...
                G.prototype._createTransceiver = function(z) {
                    ...
                    if (this.usingBundle && V)
                        q.iceTransport = this.transceivers[0].iceTransport,
                ...
                G.prototype.addTrack = function(z, V) {
                    for (var H = 0, q; H < this.transceivers.length; H++)
                        this.transceivers[H].track || this.transceivers[H].kind !== z.kind || (q = this.transceivers[H]);
                ...
                G.prototype.removeStream = function(z) {
                    var V = this.localStreams.indexOf(z);
                    -1 < V && (this.localStreams.splice(V, 1),
                    this._maybeFireNegotiationNeeded())
                }
                ,
                ...
                G.prototype._createIceGatherer = function(z, V) {
                    ...
                    return H.onlocalcandidate = function(J) {
                        ...
                        Y ? void 0 === H.state && (H.state = 'completed') : (X.component = 1,
                        W.candidate.candidate = N.writeCandidate(X));
                        ...
                        case 'gathering':
                            ...
                            Y || (q.dispatchEvent(W),
                            null !== q.onicecandidate && q.onicecandidate(W)),
                            Q && (q.dispatchEvent(new Event('icecandidate')),
                            null !== q.onicecandidate && q.onicecandidate(new Event('icecandidate')),
                            q.iceGatheringState = 'complete',
                            q._emitGatheringStateChange());
                            break;
                        case 'complete':
                            break;
                        default:
                ...
                G.prototype._disposeIceAndDtlsTransports = function(z) {
                    ...
                    V && (delete V.onlocalcandidate,
                    delete this.transceivers[z].iceGatherer);
                    var q = this.transceivers[z].iceTransport;
                    q && (delete q.onicestatechange,
                    delete this.transceivers[z].iceTransport);
                    var H = this.transceivers[z].dtlsTransport;
                    H && (delete H.ondtlssttatechange,
                    delete H.onerror,
                    delete this.transceivers[z].dtlsTransport)
                }
                ,
                G.prototype._transceive = function(z, V, q) {
                    var H = I(z.localCapabilities, z.remoteCapabilities);
                    V && z.rtpSender && (H.encodings = z.sendEncodingParameters,
                    H.rtcp = {
                        cname: N.localCName,
                        compound: z.rtcpParameters.compound
                    },
                    z.recvEncodingParameters.length && (H.rtcp.ssrc = z.recvEncodingParameters[0].ssrc),
                    z.rtpSender.send(H)),
                    q && z.rtpReceiver && ('video' === z.kind && z.recvEncodingParameters && 15019 > F && z.recvEncodingParameters.forEach(function(J) {
                        delete J.rtx
                    }),
                    H.encodings = z.recvEncodingParameters,
                    H.rtcp = {
                        cname: z.rtcpParameters.cname,
                        compound: z.rtcpParameters.compound
                    },
                    z.sendEncodingParameters.length && (H.rtcp.ssrc = z.sendEncodingParameters[0].ssrc),
                    z.rtpReceiver.receive(H))
                }
                ,
                G.prototype.setLocalDescription = function(z) {
                    ...
                    var H, J;
                    if ('offer' === z.type)
                        this._pendingOffer && (H = N.splitSections(z.sdp),
                        J = H.shift(),
                        ...
                        H.forEach(function(Z, Q) {
                            ...
                            if (!ce && !ne.isDatachannel) {
                                ...
                                W && (le.role = 'server'),
                                V.usingBundle && 0 !== Q || (oe.start(ae, pe, W ? 'controlling' : 'controlled'),
                                ie.start(le));
                                var ue = I(se, de);
                    ...
                    if (X) {
                        ...
                        U.setTimeout(function() {
                            Y(),
                            'new' === V.iceGatheringState && (V.iceGatheringState = 'gathering',
                            V._emitGatheringStateChange()),
                            V._emitBufferedCandidates()
                        }, 0)
                    }
                    var K = Promise.resolve();
                    return K.then(function() {
                        X || ('new' === V.iceGatheringState && (V.iceGatheringState = 'gathering',
                        V._emitGatheringStateChange()),
                        U.setTimeout(V._emitBufferedCandidates.bind(V), 500))
                    }),
                    K
                }
                ,
                G.prototype.setRemoteDescription = function(z) {
                    ...
                    switch (this.canTrickleIceCandidates = !!Z && 0 <= Z.substr(14).split(' ').indexOf('trickle'),
                    W.forEach(function(Q, ne) {
                        ...
                        if ('application' === oe && 'DTLS/SCTP' === se)
                            return void (V.transceivers[ne] = {
                                mid: pe,
                                isDatachannel: !0
                            });
                        var ke = N.parseRtpParameters(Q), le, ue, me, fe, ge, he, ye, be, ve, Ce, Se;
                        ie || (Ce = N.getIceParameters(Q, X),
                        Se = N.getDtlsParameters(Q, X),
                        Se.role = 'client'),
                        ...
                        ('offer' === z.type || 'answer' === z.type) && !ie && K && 0 < ne && V.transceivers[ne] && (V._disposeIceAndDtlsTransports(ne),
                        V.transceivers[ne].iceGatherer = V.transceivers[0].iceGatherer,
                        V.transceivers[ne].iceTransport = V.transceivers[0].iceTransport,
                        V.transceivers[ne].dtlsTransport = V.transceivers[0].dtlsTransport,
                        V.transceivers[ne].rtpSender && V.transceivers[ne].rtpSender.setTransport(V.transceivers[0].dtlsTransport),
                        V.transceivers[ne].rtpReceiver && V.transceivers[ne].rtpReceiver.setTransport(V.transceivers[0].dtlsTransport)),
                        'offer' !== z.type || ie ? 'answer' === z.type && !ie && (le = V.transceivers[ne],
                        ue = le.iceGatherer,
                        me = le.iceTransport,
                        fe = le.dtlsTransport,
                        ge = le.rtpReceiver,
                        he = le.sendEncodingParameters,
                        be = le.localCapabilities,
                        V.transceivers[ne].recvEncodingParameters = ye,
                        V.transceivers[ne].remoteCapabilities = ke,
                        V.transceivers[ne].rtcpParameters = xe,
                        (Y || Te) && we.length && me.setRemoteCandidates(we),
                        (!K || 0 === ne) && (me.start(ue, Ce, 'controlling'),
                        fe.start(Se)),
                        V._transceive(le, 'sendrecv' === de || 'recvonly' === de, 'sendrecv' === de || 'sendonly' === de),
                        ge && ('sendrecv' === de || 'sendonly' === de) ? (ve = ge.track,
                        ce ? (!H[ce.stream] && (H[ce.stream] = new U.MediaStream),
                        H[ce.stream].addTrack(ve),
                        J.push([ve, ge, H[ce.stream]])) : (!H.default && (H.default = new U.MediaStream),
                        H.default.addTrack(ve),
                        J.push([ve, ge, H.default]))) : delete le.rtpReceiver) : (le = V.transceivers[ne] || V._createTransceiver(oe),
                        le.mid = pe,
                        !le.iceGatherer && (le.iceGatherer = K && 0 < ne ? V.transceivers[0].iceGatherer : V._createIceGatherer(pe, ne)),
                        Te && (!K || 0 === ne) && le.iceTransport.setRemoteCandidates(we),
                        be = U.RTCRtpReceiver.getCapabilities(oe),
                        15019 > F && (be.codecs = be.codecs.filter(function(Ee) {
                            return 'rtx' !== Ee.name
                        })),
                        he = [{
                            ssrc: 1001 * (2 * ne + 2)
                        }],
                        ('sendrecv' === de || 'sendonly' === de) && (ge = new U.RTCRtpReceiver(le.dtlsTransport,oe),
                        ve = ge.track,
                        ce ? (!H[ce.stream] && (H[ce.stream] = new U.MediaStream,
                        Object.defineProperty(H[ce.stream], 'id', {
                            get: function() {
                                return ce.stream
                            }
                        })),
                        ...
                        J.push([ve, ge, H[ce.stream]])) : (!H.default && (H.default = new U.MediaStream),
                        H.default.addTrack(ve),
                        J.push([ve, ge, H.default]))),
                        le.localCapabilities = be,
                        le.remoteCapabilities = ke,
                        le.rtpReceiver = ge,
                        le.rtcpParameters = xe,
                        le.sendEncodingParameters = he,
                        le.recvEncodingParameters = ye,
                        V._transceive(V.transceivers[ne], !1, 'sendrecv' === de || 'sendonly' === de))
                    }),
                    ...
                    U.setTimeout(function() {
                        V && V.transceivers && V.transceivers.forEach(function(Q) {
                            Q.iceTransport && 'new' === Q.iceTransport.state && 0 < Q.iceTransport.getRemoteCandidates().length && (console.warn('Timeout for addRemoteCandidate. Consider sending an end-of-candidates notification'),
                ...
                G.prototype._updateConnectionState = function() {
                    ...
                    q.connected += q.completed,
                    V = 'new',
                    0 < q.failed ? V = 'failed' : 0 < q.connecting || 0 < q.checking ? V = 'connecting' : 0 < q.disconnected ? V = 'disconnected' : 0 < q.new ? V = 'new' : (0 < q.connected || 0 < q.completed) && (V = 'connected'),
                    V !== z.iceConnectionState) {
                ...
                G.prototype.createOffer = function() {
                    ...
                    for (this.transceivers.forEach(function(Y) {
                        'audio' === Y.kind ? (q--,
                        0 > q && (Y.wantReceive = !1)) : 'video' === Y.kind && (H--,
                        0 > H && (Y.wantReceive = !1))
                    }); 0 < q || 0 < H; )
                        0 < q && (this._createTransceiver('audio'),
                        q--),
                        0 < H && (this._createTransceiver('video'),
                        H--);
                ...
                G.prototype.createAnswer = function() {
                    ...
                    this.transceivers.forEach(function(q, H) {
                        if (q.isDatachannel)
                            return void (z += 'm=application 0 DTLS/SCTP 5000\r\nc=IN IP4 0.0.0.0\r\na=mid:' + q.mid + '\r\n');
                        if (q.stream) {
                            var J;
                ...
                G.prototype.addIceCandidate = function(z) {
                    if (!z) {
                        for (var V = 0; V < this.transceivers.length; V++)
                            if (this.transceivers[V].iceTransport.addRemoteCandidate({}),
                            this.usingBundle)
                        ...
                        if (J) {
                            ...
                            if ('tcp' === W.protocol && (0 === W.port || 9 === W.port))
                                return Promise.resolve();
                            if (W.component && '1' !== W.component && 1 !== W.component)
                                return Promise.resolve();
                    ...
                    if ('object' == typeof I && (I.RTCPeerConnection || I.mozRTCPeerConnection)) {
                        I.RTCPeerConnection || (I.RTCPeerConnection = function(z, V) {
                            if (38 > B.version && z && z.iceServers) {
                                for (var q = [], H = 0, J; H < z.iceServers.length; H++)
                                    if (J = z.iceServers[H],
                                    J.hasOwnProperty('urls'))
                                        for (var W = 0, X; W < J.urls.length; W++)
                                            X = {
                                                url: J.urls[W]
                                            },
                                            0 === J.urls[W].indexOf('turn') && (X.username = J.username,
                                            X.credential = J.credential),
                                            q.push(X);
                                    else
                                        q.push(z.iceServers[H]);
                                z.iceServers = q
                            }
                            return new I.mozRTCPeerConnection(z,V)
                        }
                        ,
                        I.RTCPeerConnection.prototype = I.mozRTCPeerConnection.prototype,
                        I.mozRTCPeerConnection.generateCertificate && Object.defineProperty(I.RTCPeerConnection, 'generateCertificate', {
                            get: function() {
                                return I.mozRTCPeerConnection.generateCertificate
                            }
                        }),
                        I.RTCSessionDescription = I.mozRTCSessionDescription,
                        I.RTCIceCandidate = I.mozRTCIceCandidate),
                        ['setLocalDescription', 'setRemoteDescription', 'addIceCandidate'].forEach(function(z) {
        ...
        10: [function(P, R) {
            ...
            R.exports = function(I) {
                var B = M.detectBrowser(I)
                  ...
                  , G = function(Y, K, Z) {
                    var Q = function(ne) {
                        if ('object' != typeof ne || ne.require)
                            return ne;
                        var ae = [];
                        return Object.keys(ne).forEach(function(oe) {
                            if ('require' !== oe && 'advanced' !== oe && 'mediaSource' !== oe) {
                                ...
                                if ((void 0 !== ie.min || void 0 !== ie.max || void 0 !== ie.exact) && ae.push(oe),
                                void 0 !== ie.exact && ('number' == typeof ie.exact ? ie.min = ie.max = ie.exact : ne[oe] = ie.exact,
                                delete ie.exact),
                                void 0 !== ie.ideal) {
                    ...
                    38 > B.version && (A('spec: ' + JSON.stringify(Y)),
                    Y.audio && (Y.audio = Q(Y.audio)),
                    Y.video && (Y.video = Q(Y.video)),
                    A('ff37: ' + JSON.stringify(Y))),
                ...
                if (49 > B.version) {
                    ...
                    N.mediaDevices.getUserMedia = function(Y) {
                        return q(Y).then(function(K) {
                            if (Y.audio && !K.getAudioTracks().length || Y.video && !K.getVideoTracks().length)
                                throw K.getTracks().forEach(function(Z) {
                                    Z.stop()
                                }),
                ...
                if (!(55 < B.version && 'autoGainControl'in N.mediaDevices.getSupportedConstraints())) {
                    var H = function(Y, K, Z) {
                        K in Y && !(Z in Y) && (Y[Z] = Y[K],
                        delete Y[K])
                    }
                      , J = N.mediaDevices.getUserMedia.bind(N.mediaDevices);
                    if (N.mediaDevices.getUserMedia = function(Y) {
                        return 'object' == typeof Y && 'object' == typeof Y.audio && (Y = JSON.parse(JSON.stringify(Y)),
                        H(Y.audio, 'autoGainControl', 'mozAutoGainControl'),
                        H(Y.audio, 'noiseSuppression', 'mozNoiseSuppression')),
                        J(Y)
                    }
                    ,
                    ...
                    if (U && U.prototype.applyConstraints) {
                        ...
                        U.prototype.applyConstraints = function(Y) {
                            return 'audio' === this.kind && 'object' == typeof Y && (Y = JSON.parse(JSON.stringify(Y)),
                            H(Y, 'autoGainControl', 'mozAutoGainControl'),
                            H(Y, 'noiseSuppression', 'mozNoiseSuppression')),
                            X.apply(this, [Y])
                        }
                    }
                }
                N.getUserMedia = function(Y, K, Z) {
                    return 44 > B.version ? G(Y, K, Z) : void (console.warn('navigator.getUserMedia has been replaced by navigator.mediaDevices.getUserMedia'),
                    N.mediaDevices.getUserMedia(Y).then(K, Z))
                }
            }
        }
        , {
            ...
            var M = !0
              , A = {
                disableLog: function(I) {
                    return 'boolean' == typeof I ? (M = I,
                    I ? 'adapter.js logging disabled' : 'adapter.js logging enabled') : new Error('Argument type: ' + typeof I + '. Please use a boolean.')
                },
                log: function() {
                ...
                detectBrowser: function(I) {
                    ...
                    if (N.browser = null,
                    N.version = null,
                    'undefined' == typeof I || !I.navigator)
                    ...
                    if (B.mozGetUserMedia)
                        ...
                        if (I.webkitRTCPeerConnection)
                            N.browser = 'chrome',
                            N.version = this.extractVersion(B.userAgent, /Chrom(e|ium)\/(\d+)\./, 2);
                        else if (B.userAgent.match(/Version\/(\d+).(\d+)/))
                            N.browser = 'safari',
    ...
    return function P(R, D, M) {
        ...
        2: [function(P, R) {
            R.exports = function(M, A, I) {
                ...
                for (var N = new Uint8Array(M), U = new Uint8Array(I - A), F = A, G = 0; F < I; F++,
                G++)
                    U[G] = N[F];
                return U.buffer
            }
        ...
        3: [function(P, R) {
            ...
            R.exports = function(A, I, B) {
                ...
                G.muted && (F.muted = !0),
                !G.audio && G.mirror && ['', 'moz', 'webkit', 'o', 'ms'].forEach(function(z) {
            ...
            (function() {
                ...
                case 'chrome':
                    if (!I || !I.shimPeerConnection)
                        return void M('Chrome shim is not included in this adapter release.');
                ...
                case 'firefox':
                    if (!N || !N.shimPeerConnection)
                        return void M('Firefox shim is not included in this adapter release.');
                    ...
                    break;
                case 'edge':
                    if (!B || !B.shimPeerConnection)
                        return void M('MS edge shim is not included in this adapter release.');
                ...
                case 'safari':
                    if (!U)
                        return void M('Safari shim is not included in this adapter release.');
              ...
              , I = {
                ...
                shimSourceObject: function() {
                    'object' == typeof window && window.HTMLMediaElement && !('srcObject'in window.HTMLMediaElement.prototype) && Object.defineProperty(window.HTMLMediaElement.prototype, 'srcObject', {
                        ...
                        set: function(B) {
                            ...
                            B ? void (this.src = URL.createObjectURL(B),
                ...
                shimPeerConnection: function() {
                    ...
                    RTCPeerConnection.prototype.addIceCandidate = function() {
                        return arguments[0] ? B.apply(this, arguments) : (arguments[1] && arguments[1].apply(null),
                        Promise.resolve())
                    }
                }
            };
        ...
        6: [function(P, R) {
            ...
            R.exports = function() {
                var A = function(G) {
                    if ('object' != typeof G || G.mandatory || G.optional)
                        return G;
                    var z = {};
                    return Object.keys(G).forEach(function(V) {
                        if ('require' !== V && 'advanced' !== V && 'mediaSource' !== V) {
                            ...
                            if (void 0 !== q.ideal) {
                                ...
                                var J = {};
                                'number' == typeof q.ideal ? (J[H('min', V)] = q.ideal,
                                z.optional.push(J),
                                J = {},
                                J[H('max', V)] = q.ideal,
                                z.optional.push(J)) : (J[H('', V)] = q.ideal,
                                z.optional.push(J))
                            }
                            void 0 !== q.exact && 'number' != typeof q.exact ? (z.mandatory = z.mandatory || {},
                            z.mandatory[H('', V)] = q.exact) : ['min', 'max'].forEach(function(W) {
                                void 0 !== q[W] && (z.mandatory = z.mandatory || {},
                                z.mandatory[H(W, V)] = q[W])
                            })
                        }
                    }),
                  ...
                  , I = function(G, z) {
                    ...
                    G && G.audio && (G.audio = A(G.audio)),
                    G && 'object' == typeof G.video) {
                        var V = G.video.facingMode;
                        if (V = V && ('object' == typeof V ? V : {
                            ideal: V
                        }),
                        V && ('user' === V.exact || 'environment' === V.exact || 'user' === V.ideal || 'environment' === V.ideal) && !(navigator.mediaDevices.getSupportedConstraints && navigator.mediaDevices.getSupportedConstraints().facingMode) && (delete G.video.facingMode,
                        'environment' === V.exact || 'environment' === V.ideal))
                            return navigator.mediaDevices.enumerateDevices().then(function(q) {
                    ...
                    navigator.mediaDevices.getUserMedia = function(G) {
                        return I(G, function(z) {
                            return F(z).then(function(V) {
                                if (z.audio && !V.getAudioTracks().length || z.video && !V.getVideoTracks().length)
                                    throw V.getTracks().forEach(function(q) {
                                        q.stop()
                                    }),
                                    new DOMException('','NotFoundError');
                                return V
                ...
                shimPeerConnection: {
                    shimPeerConnection: function() {
                        ...
                        window.RTCPeerConnection = function(N) {
                            ...
                            N && N.iceTransportPolicy)
                                switch (N.iceTransportPolicy) {
                                case 'all':
                                case 'relay':
                            ...
                            if (this.usingBundle = N && 'max-bundle' === N.bundlePolicy,
                            N && N.iceServers) {
                        ...
                        window.RTCPeerConnection.prototype._emitBufferedCandidates = function() {
                            ...
                            this._localIceCandidatesBuffer.forEach(function(F) {
                                ...
                                if (G)
                                    ...
                                    -1 === F.candidate.candidate.indexOf('typ endOfCandidates') && (U[F.candidate.sdpMLineIndex + 1] += 'a=' + F.candidate.candidate + '\r\n');
                                if (N.localDescription.sdp = U.join(''),
                                N.dispatchEvent(F),
                                null !== N.onicecandidate && N.onicecandidate(F),
                                !F.candidate && 'complete' !== N.iceGatheringState) {
                        ...
                        window.RTCPeerConnection.prototype.removeStream = function(N) {
                            var U = this.localStreams.indexOf(N);
                            -1 < U && (this.localStreams.splice(U, 1),
                            this._maybeFireNegotiationNeeded())
                        }
                        ,
                        ...
                        window.RTCPeerConnection.prototype._getCommonCapabilities = function(N, U) {
                            ...
                            return N.codecs.forEach(function(G) {
                                for (var z = 0, V; z < U.codecs.length; z++)
                                    if (V = U.codecs[z],
                                    G.name.toLowerCase() === V.name.toLowerCase() && G.clockRate === V.clockRate) {
                                        ...
                                        V.rtcpFeedback = V.rtcpFeedback.filter(function(q) {
                                            for (var H = 0; H < G.rtcpFeedback.length; H++)
                                                if (G.rtcpFeedback[H].type === q.type && G.rtcpFeedback[H].parameter === q.parameter)
                                                    return !0;
                                            return !1
                                        });
                                        break
                        ...
                        window.RTCPeerConnection.prototype._createIceAndDtlsTransports = function(N, U) {
                            ...
                            G.onlocalcandidate = function(q) {
                                ...
                                W ? (void 0 === G.state && (G.state = 'completed'),
                                H.candidate.candidate = 'candidate:1 1 udp 1 0.0.0.0 9 typ endOfCandidates') : (J.component = 'RTCP' === z.component ? 2 : 1,
                                H.candidate.candidate = M.writeCandidate(J));
                                ...
                                case 'gathering':
                                    ...
                                    Y && (F.dispatchEvent(new Event('icecandidate')),
                                    null !== F.onicecandidate && F.onicecandidate(new Event('icecandidate')),
                                    F.iceGatheringState = 'complete');
                                    break;
                                case 'complete':
                                    break;
                                default:
                        ...
                        window.RTCPeerConnection.prototype._transceive = function(N, U, F) {
                            ...
                            U && N.rtpSender && (G.encodings = N.sendEncodingParameters,
                            G.rtcp = {
                                cname: M.localCName
                            },
                            N.recvEncodingParameters.length && (G.rtcp.ssrc = N.recvEncodingParameters[0].ssrc),
                            N.rtpSender.send(G)),
                            F && N.rtpReceiver && ('video' === N.kind && N.recvEncodingParameters && N.recvEncodingParameters.forEach(function(z) {
                                delete z.rtx
                            }),
                            G.encodings = N.recvEncodingParameters,
                            G.rtcp = {
                                cname: N.cname
                            },
                            N.sendEncodingParameters.length && (G.rtcp.ssrc = N.sendEncodingParameters[0].ssrc),
                            N.rtpReceiver.receive(G))
                        }
                        ,
                        window.RTCPeerConnection.prototype.setLocalDescription = function(N) {
                            var U = this, F, G;
                            if ('offer' === N.type)
                                this._pendingOffer && (F = M.splitSections(N.sdp),
                                G = F.shift(),
                                ...
                                F.forEach(function(J, W) {
                                    ...
                                    if (!ae && !X.isDatachannel) {
                                        ...
                                        z && (se.role = 'server'),
                                        U.usingBundle && 0 !== W || (K.start(Y, oe, z ? 'controlling' : 'controlled'),
                                        Z.start(se));
                            ...
                            if (V) {
                                ...
                                window.setTimeout(function() {
                                    q(),
                                    'new' === U.iceGatheringState && (U.iceGatheringState = 'gathering'),
                                    U._emitBufferedCandidates()
                                }, 0)
                            }
                            var H = Promise.resolve();
                            return H.then(function() {
                                V || ('new' === U.iceGatheringState && (U.iceGatheringState = 'gathering'),
                                window.setTimeout(U._emitBufferedCandidates.bind(U), 500))
                            }),
                            H
                        }
                        ,
                        window.RTCPeerConnection.prototype.setRemoteDescription = function(N) {
                            ...
                            switch (this.usingBundle = 0 < M.matchPrefix(V, 'a=group:BUNDLE ').length,
                            z.forEach(function(H, J) {
                                ...
                                if (Q = Q.length ? Q[0].substr(6) : M.generateIdentifier(),
                                'application' === Y && 'DTLS/SCTP' === X[2])
                                    return void (U.transceivers[J] = {
                                        mid: Q,
                                        isDatachannel: !0
                                    });
                                var me = M.parseRtpParameters(H), ne, ae, oe, ie, se, de, ce, pe, le, ue, fe, ge;
                                K || (fe = M.getIceParameters(H, V),
                                ge = M.getDtlsParameters(H, V),
                                ge.role = 'client'),
                                ...
                                if ('offer' === N.type && !K) {
                                    ...
                                    if (be && ke.iceTransport.setRemoteCandidates(ve),
                                    le = RTCRtpReceiver.getCapabilities(Y),
                                    le.codecs = le.codecs.filter(function(Se) {
                                        return 'rtx' !== Se.name
                                    }),
                                    ce = [{
                                        ssrc: 1001 * (2 * J + 2)
                                    }],
                                    de = new RTCRtpReceiver(ke.dtlsTransport,Y),
                                    ue = de.track,
                                    G.push([ue, de]),
                                    F.addTrack(ue),
                                    0 < U.localStreams.length && U.localStreams[0].getTracks().length >= J) {
                                        var Ce;
                                    ...
                                    'answer' !== N.type || K || (ne = U.transceivers[J],
                                    ae = ne.iceGatherer,
                                    oe = ne.iceTransport,
                                    ie = ne.dtlsTransport,
                                    se = ne.rtpSender,
                                    de = ne.rtpReceiver,
                                    ce = ne.sendEncodingParameters,
                                    le = ne.localCapabilities,
                                    U.transceivers[J].recvEncodingParameters = pe,
                                    U.transceivers[J].remoteCapabilities = me,
                                    U.transceivers[J].cname = he,
                                    (q || be) && ve.length && oe.setRemoteCandidates(ve),
                                    U.usingBundle && 0 !== J || (oe.start(ae, fe, 'controlling'),
                                    ie.start(ge)),
                                    U._transceive(ne, 'sendrecv' === Z || 'recvonly' === Z, 'sendrecv' === Z || 'sendonly' === Z),
                                    de && ('sendrecv' === Z || 'sendonly' === Z) ? (ue = de.track,
                                    G.push([ue, de]),
                                    F.addTrack(ue)) : delete ne.rtpReceiver)
                            }),
                            this.remoteDescription = {
                                type: N.type,
                                sdp: N.sdp
                            ...
                            return F.getTracks().length && (U.remoteStreams.push(F),
                            window.setTimeout(function() {
                        ...
                        window.RTCPeerConnection.prototype._updateConnectionState = function() {
                            ...
                            F.connected += F.completed,
                            U = 'new',
                            0 < F.failed ? U = 'failed' : 0 < F.connecting || 0 < F.checking ? U = 'connecting' : 0 < F.disconnected ? U = 'disconnected' : 0 < F.new ? U = 'new' : (0 < F.connected || 0 < F.completed) && (U = 'connected'),
                            U !== N.iceConnectionState) {
                        ...
                        window.RTCPeerConnection.prototype.createOffer = function() {
                            ...
                            var F = []
                              , G = 0
                              , z = 0;
                            if (this.localStreams.length && (G = this.localStreams[0].getAudioTracks().length,
                            z = this.localStreams[0].getVideoTracks().length),
                            U) {
                        ...
                        window.RTCPeerConnection.prototype.createAnswer = function() {
                            ...
                            this.transceivers.forEach(function(G) {
                                if (G.isDatachannel)
                                    return void (U += 'm=application 0 DTLS/SCTP 5000\r\nc=IN IP4 0.0.0.0\r\na=mid:' + G.mid + '\r\n');
                        ...
                        window.RTCPeerConnection.prototype.addIceCandidate = function(N) {
                            if (!N)
                                ...
                                if (G) {
                                    ...
                                    if ('tcp' === z.protocol && (0 === z.port || 9 === z.port))
                                        return;
                                    if ('1' !== z.component)
                                        return;
              ...
              , A = {
                ...
                shimPeerConnection: function() {
                    if ('object' == typeof window && (window.RTCPeerConnection || window.mozRTCPeerConnection)) {
                        window.RTCPeerConnection || (window.RTCPeerConnection = function(U, F) {
                            if (38 > M.version && U && U.iceServers) {
                                for (var G = [], z = 0, V; z < U.iceServers.length; z++)
                                    if (V = U.iceServers[z],
                                    V.hasOwnProperty('urls'))
                                        for (var q = 0, H; q < V.urls.length; q++)
                                            H = {
                                                url: V.urls[q]
                                            },
                                            0 === V.urls[q].indexOf('turn') && (H.username = V.username,
                                            H.credential = V.credential),
                                            G.push(H);
                                    else
                                        G.push(U.iceServers[z]);
                                U.iceServers = G
                            }
                        ...
                        ['setLocalDescription', 'setRemoteDescription', 'addIceCandidate'].forEach(function(U) {
                        ...
                        if (RTCPeerConnection.prototype.addIceCandidate = function() {
                            return arguments[0] ? I.apply(this, arguments) : (arguments[1] && arguments[1].apply(null),
                            Promise.resolve())
                        }
                        ,
                        48 > M.version) {
            ...
            R.exports = function() {
                var I = function(G) {
                  ...
                  , B = function(G, z, V) {
                    var q = function(H) {
                        ...
                        return Object.keys(H).forEach(function(W) {
                            if ('require' !== W && 'advanced' !== W && 'mediaSource' !== W) {
                                ...
                                if ((void 0 !== X.min || void 0 !== X.max || void 0 !== X.exact) && J.push(W),
                                void 0 !== X.exact && ('number' == typeof X.exact ? X.min = X.max = X.exact : H[W] = X.exact,
                                delete X.exact),
                                void 0 !== X.ideal) {
                    ...
                    38 > A.version && (M('spec: ' + JSON.stringify(G)),
                    G.audio && (G.audio = q(G.audio)),
                    G.video && (G.video = q(G.video)),
                    M('ff37: ' + JSON.stringify(G))),
                ...
                if (49 > A.version) {
                    ...
                    navigator.mediaDevices.getUserMedia = function(G) {
                        return F(G).then(function(z) {
                            if (G.audio && !z.getAudioTracks().length || G.video && !z.getVideoTracks().length)
                                throw z.getTracks().forEach(function(V) {
                                    V.stop()
                                }),
                                new DOMException('The object can not be found here.','NotFoundError');
                            return z
                ...
                navigator.getUserMedia = function(G, z, V) {
                    return 44 > A.version ? B(G, z, V) : void (console.warn('navigator.getUserMedia has been replaced by navigator.mediaDevices.getUserMedia'),
                    navigator.mediaDevices.getUserMedia(G).then(z, V))
                }
            }
        }
        , {
                ...
                detectBrowser: function() {
                    ...
                    if (navigator.mozGetUserMedia)
                        ...
                        if (window.webkitRTCPeerConnection)
                            ...
                            return I.browser = 'Unsupported webkit-based browser with GUM support but no WebRTC support.',
                            I;
        ...
        14: [function(P, R, D) {
            (function(M) {
                'use strict';
                D.encode = function(A) {
                    var I = new Uint8Array(A), N = I.length, U = '', B;
                    for (B = 0; B < N; B += 3)
                        U += M[I[B] >> 2],
                        U += M[(3 & I[B]) << 4 | I[B + 1] >> 4],
                ...
                ,
                D.decode = function(A) {
                    var I = 0.75 * A.length, B = A.length, U = 0, N, F, G, z, V;
                    '=' === A[A.length - 1] && (I--,
                    '=' === A[A.length - 2] && I--);
            ...
            (function(M) {
                function A(z) {
                    for (var V = 0, q; V < z.length; V++)
                        if (q = z[V],
                        q.buffer instanceof ArrayBuffer) {
                            var H = q.buffer;
        ...
        17: [function(P, R) {
            ...
            M.prototype.on = M.prototype.addEventListener = function(I, B) {
                ...
                (this._callbacks[I] = this._callbacks[I] || []).push(B),
                this
            }
            ,
            ...
            M.prototype.off = M.prototype.removeListener = M.prototype.removeAllListeners = M.prototype.removeEventListener = function(I, B) {
                ...
                if (1 == arguments.length)
                    return delete this._callbacks[I],
                    this;
        ...
        19: [function(P, R) {
            ...
            M.enable = function(I) {
                ...
                for (var B = (I || '').split(/[\s,]+/), N = B.length, U = 0; U < N; U++)
                    I = B[U].replace('*', '.*?'),
                    '-' === I[0] ? M.skips.push(new RegExp('^' + I.substr(1) + '$')) : M.names.push(new RegExp('^' + I + '$'))
            }
            ,
            M.disable = function() {
                M.enable('')
            ...
            (function(M) {
                function I(J, W) {
                    if (!(this instanceof I))
                        return new I(J,W);
                    if (W = W || {},
                    J && 'object' == typeof J && (W = J,
                    J = null),
                    J && (J = V(J),
                    W.host = J.host,
                    W.secure = 'https' == J.protocol || 'wss' == J.protocol,
                    W.port = J.port,
                    J.query && (W.query = J.query)),
                    this.secure = null == W.secure ? M.location && 'https:' == location.protocol : W.secure,
                    W.host) {
                        var X = W.host.split(':');
                        W.hostname = X.shift(),
                ...
                I.prototype.probe = function(J) {
                    function W() {
                        ...
                        oe || (F('probe transport "%s" opened', J),
                        ae.send([{
                            type: 'ping',
                            data: 'probe'
                        }]),
                        ae.once('packet', function(de) {
                            if (!oe)
                                if ('pong' == de.type && 'probe' == de.data) {
                                    if (F('probe transport "%s" pong', J),
                                    ie.upgrading = !0,
                                    ie.emit('upgrading', ae),
                                    !ae)
                                        return;
                                    I.priorWebsocketSuccess = 'websocket' == ae.name,
                                    F('pausing current transport "%s"', ie.transport.name),
                                    ie.transport.pause(function() {
                                        oe || 'closed' == ie.readyState || (F('changing transport and sending upgrade packet'),
                                        ne(),
                                        ie.setTransport(ae),
                                        ae.send([{
                                            type: 'upgrade'
                    ...
                    function Q(se) {
                        ae && se.name != ae.name && (F('"%s" works - aborting "%s"', se.name, ae.name),
                        X())
                    }
                    function ne() {
                ...
                I.prototype.onOpen = function() {
                    ...
                    'open' == this.readyState && this.upgrade && this.transport.pause) {
                        ...
                        for (var J = 0, W = this.upgrades.length; J < W; J++)
                ...
                I.prototype.onPacket = function(J) {
                    if ('opening' == this.readyState || 'open' == this.readyState)
                        switch (F('socket receive: type "%s", data "%s"', J.type, J.data),
                        this.emit('packet', J),
                        this.emit('heartbeat'),
                        J.type) {
                        case 'open':
                ...
                I.prototype.onDrain = function() {
                    for (var J = 0; J < this.prevBufferLen; J++)
                        this.callbackBuffer[J] && this.callbackBuffer[J]();
                ...
                I.prototype.filterUpgrades = function(J) {
                    for (var W = [], X = 0, Y = J.length; X < Y; X++)
                        ~G(this.transports, J[X]) && W.push(J[X]);
                    return W
                }
            }
            ).call(this, 'undefined' == typeof global ? 'undefined' == typeof self ? 'undefined' == typeof window ? {} : window : self : global)
        ...
        25: [function(P, R) {
            (function(M) {
                ...
                I.prototype.doWrite = function(V, q) {
                    ...
                    var W = this;
                    if (!this.form) {
                        var X = document.createElement('form'), Y = document.createElement('textarea'), K = this.iframeId = 'eio_iframe_' + this.index, Z;
        ...
        26: [function(P, R) {
            (function(M) {
                ...
                function N() {
                    for (var q in B.requests)
                        B.requests.hasOwnProperty(q) && B.requests[q].abort()
                }
                ...
                B.prototype.create = function() {
                    var q = {
                      ...
                      , J = this;
                    try {
                        if (V('xhr open %s: %s', this.method, this.uri),
                    ...
                    M.document && (this.index = B.requestsCount++,
                ...
                ,
                M.document && (B.requestsCount = 0,
                B.requests = {},
                M.attachEvent ? M.attachEvent('onunload', N) : M.addEventListener && M.addEventListener('beforeunload', N, !1))
            }
            ).call(this, 'undefined' == typeof global ? 'undefined' == typeof self ? 'undefined' == typeof window ? {} : window : self : global)
        }
        , {
        ...
        27: [function(P, R) {
            ...
            M.prototype.onData = function(G) {
                ...
                B.decodePayload(G, this.socket.binaryType, function(q) {
                    return 'opening' == z.readyState && z.onOpen(),
                    'close' == q.type ? (z.onClose(),
                    !1) : void z.onPacket(q)
                }),
            ...
            M.prototype.doClose = function() {
                ...
                G()) : (U('transport not open - deferring close'),
                this.once('open', G))
            }
            ,
        ...
        28: [function(P, R) {
            ...
            M.prototype.write = function(G) {
                ...
                for (var q = 0, H = G.length; q < H; q++)
                    I.encodePacket(G[q], this.supportsBinary, function(J) {
                        try {
                            V.ws.send(J)
                        } catch (W) {
        ...
        29: [function(P, R) {
            ...
            R.exports = function(A) {
                var I = A.xdomain
                  ...
                  , N = A.enablesXDR;
                try {
                    if ('undefined' != typeof XMLHttpRequest && (!I || M))
                        return new XMLHttpRequest
                } catch (U) {}
                try {
                    if ('undefined' != typeof XDomainRequest && !B && N)
        ...
        30: [function(P, R, D) {
            ...
            D.formatArgs = function() {
                var U = arguments
                  , F = this.useColors;
                if (U[0] = (F ? '%c' : '') + this.namespace + (F ? ' %c' : ' ') + U[0] + (F ? '%c ' : ' ') + '+' + D.humanize(this.diff),
                !F)
                    return U;
        ...
        31: [function(P, R, D) {
            ...
            D = R.exports = function(z) {
                ...
                function q() {
                    ...
                    Y[0] = D.coerce(Y[0]),
                    'string' != typeof Y[0] && (Y = ['%o'].concat(Y));
                    var K = 0;
            ...
            D.enable = function(z) {
                D.save(z);
                for (var V = (z || '').split(/[\s,]+/), q = V.length, H = 0; H < q; H++)
                    V[H] && (z = V[H].replace(/\*/g, '.*?'),
                    '-' === z[0] ? D.skips.push(new RegExp('^' + z.substr(1) + '$')) : D.names.push(new RegExp('^' + z + '$')))
            }
            ,
            D.enabled = function(z) {
                var V, q;
                for (V = 0,
                q = D.skips.length; V < q; V++)
                    if (D.skips[V].test(z))
                        return !1;
                for (V = 0,
                q = D.names.length; V < q; V++)
                    if (D.names[V].test(z))
                        return !0;
                return !1
            }
        ...
        32: [function(P, R) {
            ...
            R.exports = function(B) {
                var N = B
                  , U = B.indexOf('[')
                  , F = B.indexOf(']');
                -1 != U && -1 != F && (B = B.substring(0, U) + B.substring(U, F).replace(/:/g, ';') + B.substring(F, B.length));
                for (var G = M.exec(B || ''), z = {}, V = 14; V--; )
                    z[A[V]] = G[V] || '';
                return -1 != U && -1 != F && (z.source = N,
                z.host = z.host.substring(1, z.host.length - 1).replace(/;/g, ':'),
        ...
        33: [function(P, R, D) {
            (function(M) {
                ...
                function I(ne, ae, oe) {
                    if (!ae)
                        return D.encodeBase64Packet(ne, oe);
                    var ie = ne.data
                ...
                D.encodePacket = function(ne, ae, oe, ie) {
                    'function' == typeof ae && (ie = ae,
                    ae = !1),
                    'function' == typeof oe && (ie = oe,
                    oe = null);
                    var se = void 0 === ne.data ? void 0 : ne.data.buffer || ne.data;
                    if (M.ArrayBuffer && se instanceof ArrayBuffer)
                        return I(ne, ae, ie);
                    if (Q && se instanceof M.Blob)
                        return N(ne, ae, ie);
                    if (se && se.base64)
                        return A(ne, ie);
                    var de = Y[ne.type];
                ...
                D.encodeBase64Packet = function(ne, ae) {
                    ...
                    try {
                        ...
                        for (var de = new Uint8Array(ne.data), ce = Array(de.length), pe = 0; pe < de.length; pe++)
                            ce[pe] = de[pe];
                ...
                D.decodePacket = function(ne, ae, oe) {
                    if ('string' == typeof ne || void 0 === ne) {
                        if ('b' == ne.charAt(0))
                            return D.decodeBase64Packet(ne.substr(1), ae);
                        if (oe)
                            try {
                                ne = H.decode(ne)
                            } catch (ce) {
                ...
                D.decodePayload = function(ne, ae, oe) {
                    if ('string' != typeof ne)
                        return D.decodePayloadAsBinary(ne, ae, oe);
                    'function' == typeof ae && (oe = ae,
                    ae = null);
                    var ie;
                    if ('' == ne)
                    ...
                    for (var se = '', pe = 0, le = ne.length, de, ce, ue; pe < le; pe++)
                        ...
                        ':' != ue)
                            ...
                            if ('' == se || se != (de = +se))
                                return oe(Z, 0, 1);
                            if (ce = ne.substr(pe + 1, de),
                            se != ce.length)
                                return oe(Z, 0, 1);
                            if (ce.length) {
                                if (ie = D.decodePacket(ce, ae, !0),
                                Z.type == ie.type && Z.data == ie.data)
                                    return oe(Z, 0, 1);
                    ...
                    return ne.length ? void U(ne, function(ie, se) {
                        ...
                        return se.forEach(function(le) {
                            var ue = 'string' == typeof le
                              , me = le;
                            if (ue) {
                                for (var fe = new Uint8Array(le.length), ge = 0; ge < le.length; ge++)
                                    fe[ge] = le.charCodeAt(ge);
                                me = fe.buffer
                            }
                            ce[pe++] = ue ? 0 : 1;
                            for (var he = me.byteLength.toString(), ge = 0; ge < he.length; ge++)
                                ce[pe++] = parseInt(he[ge]);
                            ce[pe++] = 255;
                            for (var fe = new Uint8Array(me), ge = 0; ge < fe.length; ge++)
                                ce[pe++] = fe[ge]
                        }),
                        ae(ce.buffer)
                ...
                D.encodePayloadAsBlob = function(ne, ae) {
                    U(ne, function(ie, se) {
                        D.encodePacket(ie, !0, !0, function(de) {
                            ...
                            if (ce[0] = 1,
                            'string' == typeof de) {
                                for (var pe = new Uint8Array(de.length), le = 0; le < de.length; le++)
                                    pe[le] = de.charCodeAt(le);
                                de = pe.buffer,
                                ce[0] = 0
                            }
                            for (var ue = de instanceof ArrayBuffer ? de.byteLength : de.size, me = ue.toString(), fe = new Uint8Array(me.length + 1), le = 0; le < me.length; le++)
                                fe[le] = parseInt(me[le]);
                            if (fe[me.length] = 255,
                            Q) {
        ...
        35: [function(P, R) {
            ...
            M.prototype.send = function(N, U) {
                  ...
                  , V = function() {
                    ...
                    q.onload = function() {
                        return function(J) {
                            ...
                            N.size > z + J.target.result.byteLength ? G ? window.setTimeout(V, F.config.pacing) : U.bufferedAmount <= U.bufferedAmountLowThreshold && window.setTimeout(V, 0) : (F.emit('progress', N.size, N.size, null),
                            F.emit('sentFile')),
                ...
                G || (U.bufferedAmountLowThreshold = 8 * this.config.chunksize,
                U.addEventListener('bufferedamountlow', V)),
            ...
            A.prototype.receive = function(N, U) {
                ...
                this.channel.onmessage = function(G) {
                    ...
                    F.received === F.metadata.size ? (F.emit('receivedFile', new window.Blob(F.receiveBuffer), F.metadata),
                    F.receiveBuffer = []) : F.received > F.metadata.size && (console.error('received more than expected, discarding...'),
                    F.receiveBuffer = [])
                }
            }
            ,
        ...
        36: [function(P, R) {
            var M = {};
            R.exports = function(A, I) {
                var B = 2 === arguments.length, N = B ? I : A, U;
                ...
                if (window.navigator.userAgent.match('Chrome')) {
                    ...
                    if (window.navigator.userAgent.match('Linux') && (G = 35),
                    sessionStorage.getScreenMediaJSExtensionId)
                        ...
                        window.cefGetScreenMedia(function(H) {
                            if (!H) {
                    ...
                    window.navigator.mediaDevices.getUserMedia(A).then(function(H) {
                        ...
                        var J = H.currentTime
                          , W = window.setInterval(function() {
                            ...
                            H.currentTime == J && (window.clearInterval(W),
                            H.onended && H.onended()),
                            J = H.currentTime
                        }, 500)
                    }).catch(function(H) {
                        N(H)
                    })) : (U = new Error('NavigatorUserMediaError'),
                    U.name = 'EXTENSION_UNAVAILABLE')
                }
            }
            ,
            'undefined' != typeof window && window.addEventListener('message', function(A) {
                if (A.origin == window.location.origin)
                    if ('gotScreen' == A.data.type && M[A.data.id]) {
                        ...
                        'getScreenPending' == A.data.type && window.clearTimeout(A.data.id)
            })
        }
        , {}],
        37: [function(P, R) {
        ...
        38: [function(P, R) {
            function M(N, U) {
                ...
                for (var G = 4, z = U.length; G < z; G++)
                    U[G] > F && 0 > U[G] && (F = U[G]);
                return F
            }
            var A = P('wildemitter'), I;
            ...
            R.exports = function(N, U) {
                ...
                N.jquery && (N = N[0]),
                N instanceof HTMLAudioElement || N instanceof HTMLVideoElement ? (W = B.createMediaElementSource(N),
                'undefined' == typeof q && (q = !0),
                V = V || -50) : (W = B.createMediaStreamSource(N),
                V = V || -50),
                W.connect(Y),
                ...
                F.stop = function() {
                    ...
                    F.speaking && (F.speaking = !1,
                    F.emit('stopped_speaking')),
                    Y.disconnect(),
                    W.disconnect()
                }
                ,
                ...
                var Z = function() {
                    setTimeout(function() {
                        if (J) {
                            ...
                            var ne = 0;
                            if (Q > V && !F.speaking) {
                                for (var ae = F.speakingHistory.length - 3; ae < F.speakingHistory.length; ae++)
                                    ne += F.speakingHistory[ae];
                                2 <= ne && (F.speaking = !0,
                                F.emit('speaking'))
                            } else if (Q < V && F.speaking) {
                                for (var ae = 0; ae < F.speakingHistory.length; ae++)
                                    ne += F.speakingHistory[ae];
                                0 == ne && (F.speaking = !1,
                                F.emit('stopped_speaking'))
                            }
            ...
            (function(M) {
                ...
                R.exports = function(B) {
                    function N(U) {
                        if (!U)
                            return !1;
                        if (M.Buffer && M.Buffer.isBuffer(U) || M.ArrayBuffer && U instanceof ArrayBuffer || M.Blob && U instanceof Blob || M.File && U instanceof File)
                            return !0;
                        if (I(U)) {
                            ...
                            U)
                                if (Object.prototype.hasOwnProperty.call(U, G) && N(U[G]))
                                    return !0;
                        return !1
                    }
                    return N(B)
            ...
            (function(M) {
                function A(Ce) {
                    if (A[Ce] !== U)
                        return A[Ce];
                    var Se;
                    if ('bug-string-char-index' == Ce)
                        Se = 'a' != 'a'[0];
                    else if ('json' == Ce)
                        Se = A('json-stringify') && A('json-parse');
                    else {
                ...
                var I = {}.toString, B, N, U, F = 'function' == typeof C && C.amd, G = 'object' == typeof JSON && JSON, z = 'object' == typeof D && D && !D.nodeType && D;
                z && G ? (z.stringify = G.stringify,
                z.parse = G.parse) : z = M.JSON = G || {};
                ...
                if (!A('json')) {
                    ...
                    (B = {}.hasOwnProperty) || (B = function(Ce) {
                        ...
                        Se).toString == I ? (xe = Se.constructor,
                        B = function(Te) {
                    ...
                    if (N = function(Ce, Se) {
                        ...
                        xe ? 2 == xe ? N = function(Pe, Re) {
                            var _e = {}, De = I.call(Pe) == q, Me;
                            for (Me in Pe)
                                !(De && 'prototype' == Me) && !B.call(_e, Me) && (_e[Me] = 1) && B.call(Pe, Me) && Re(Me)
                        }
                        : N = function(Pe, Re) {
                            var _e = I.call(Pe) == q, De, Me;
                            for (De in Pe)
                                _e && 'prototype' == De || !B.call(Pe, De) || (Me = 'constructor' === De) || Re(De);
                            (Me || B.call(Pe, De = 'constructor')) && Re(De)
                        }
                        : (we = ['valueOf', 'toString', 'toLocaleString', 'propertyIsEnumerable', 'isPrototypeOf', 'hasOwnProperty', 'constructor'],
                        N = function(Pe, Re) {
                            var _e = I.call(Pe) == q, je = !_e && 'function' != typeof Pe.constructor && oe(Pe, 'hasOwnProperty') ? Pe.hasOwnProperty : B, De, Me;
                            for (De in Pe)
                                !(_e && 'prototype' == De) && je.call(Pe, De) && Re(De);
                    ...
                    !A('json-stringify')) {
                        var ie = {
                          ...
                          , pe = function(Ce) {
                            var Se = '"', xe = 0, Te = Ce.length, we = 10 < Te && K, Ee;
                          ...
                          , le = function(Ce, Se, xe, Te, we, Ee, Pe) {
                            ...
                            if ('object' == typeof Re && Re)
                                if (_e = I.call(Re),
                                _e != '[object Date]' || B.call(Re, 'toJSON'))
                                    'function' == typeof Re.toJSON && (_e != J && _e != W && _e != X || B.call(Re, 'toJSON')) && (Re = Re.toJSON(Ce));
                                else if (Re > -1 / 0 && Re < 1 / 0) {
                                    if (ne) {
                                        for (je = Z(Re / 864e5),
                                        De = Z(je / 365.2425) + 1970 - 1; ne(De + 1, 0) <= je; De++)
                                            ;
                            ...
                            if ('object' == typeof Re) {
                                ...
                                _e == X) {
                                    for (Fe = 0,
                                    Ge = Re.length; Fe < Ge; Fe++)
                                        Ue = le(Fe, Re, xe, Te, we, Ee, Pe),
                        ...
                        z.stringify = function(Ce, Se, xe) {
                            ...
                            if ('function' == typeof Se || 'object' == typeof Se && Se)
                                if ((Pe = I.call(Se)) == q)
                                    ...
                                    Ee = {};
                                    for (var Re = 0, _e = Se.length, De; Re < _e; De = Se[Re++],
                                    (Pe = I.call(De),
                                    Pe == W || Pe == J) && (Ee[De] = 1))
                                        ;
                                }
                            if (xe)
                                if ((Pe = I.call(xe)) != J)
                                    Pe == W && (Te = 10 >= xe.length ? xe : xe.slice(0, 10));
                                else if (0 < (xe -= xe % 1))
                                    for (Te = '',
                                    10 < xe && (xe = 10); Te.length < xe; Te += ' ')
                                        ;
                            return le('', (De = {},
                            De[''] = Ce,
                            De), we, Ee, Te, '', [])
                    ...
                    if (!A('json-parse')) {
                        var ue = String.fromCharCode, me = {
                            ...
                            for (var Ce = ge, Se = Ce.length, xe, Te, we, Ee, Pe; fe < Se; )
                                switch (Pe = Ce.charCodeAt(fe),
                                Pe) {
                                case 9:
                                case 10:
                                case 13:
                                ...
                                case 34:
                                    ...
                                    fe++; fe < Se; )
                                        ...
                                        32 > Pe)
                                            ...
                                            if (34 == Pe)
                                                break;
                                            for (Pe = Ce.charCodeAt(fe),
                                            Te = fe; 32 <= Pe && 92 != Pe && 34 != Pe; )
                                                Pe = Ce.charCodeAt(++fe);
                                            xe += Ce.slice(Te, fe)
                                        }
                                    if (34 == Ce.charCodeAt(fe))
                                        return fe++,
                                        xe;
                                    he();
                                default:
                                    ...
                                    Pe = Ce.charCodeAt(++fe)),
                                    48 <= Pe && 57 >= Pe) {
                                        for (48 == Pe && (Pe = Ce.charCodeAt(fe + 1),
                                        48 <= Pe && 57 >= Pe) && he(),
                                        Ee = !1; fe < Se && (Pe = Ce.charCodeAt(fe),
                                        48 <= Pe && 57 >= Pe); fe++)
                                            ;
                                        if (46 == Ce.charCodeAt(fe)) {
                                            for (we = ++fe; we < Se && (Pe = Ce.charCodeAt(we),
                                            48 <= Pe && 57 >= Pe); we++)
                                                ;
                                            we == fe && he(),
                                            fe = we
                                        ...
                                        101 == Pe || 69 == Pe) {
                                            for (Pe = Ce.charCodeAt(++fe),
                                            (43 == Pe || 45 == Pe) && fe++,
                                            we = fe; we < Se && (Pe = Ce.charCodeAt(we),
                                            48 <= Pe && 57 >= Pe); we++)
                                                ;
                                            we == fe && he(),
                                            fe = we
                                    ...
                                    if (Ee && he(),
                                    'true' == Ce.slice(fe, fe + 4))
                                        return fe += 4,
                                        !0;
                                    if ('false' == Ce.slice(fe, fe + 5))
                                        return fe += 5,
                                        !1;
                                    if ('null' == Ce.slice(fe, fe + 4))
                                        return fe += 4,
                                        null;
                                    he();
                                }
                            ...
                            'string' == typeof Ce) {
                                ...
                                if ('{' == Ce) {
                                    for (Se = {}; ; xe || (xe = !0)) {
                                        ...
                                        xe && (',' == Ce ? (Ce = ye(),
                                        '}' == Ce && he()) : he()),
                                        (',' == Ce || 'string' != typeof Ce || '@' != (K ? Ce.charAt(0) : Ce[0]) || ':' != ye()) && he(),
                                        Se[Ce.slice(1)] = be(ye())
                                    }
                                    return Se
                                }
        ...
        44: [function(P, R) {
            ...
            function I(z) {
                ...
                for (q in z)
                    z.hasOwnProperty(q) && (this.config[q] = z[q]);
            ...
            I.prototype.start = function(z, V) {
                ...
                navigator.mediaDevices.getUserMedia(H).then(function(J) {
                    if (H.audio && q.config.detectSpeakingEvents && q._setupAudioMonitor(J, q.config.harkOptions),
                    q.localStreams.push(J),
                    J.getTracks().forEach(function(W) {
                        W.addEventListener('ended', function() {
                            M(J) && q._removeStream(J)
                        })
                    }),
                    q.emit('localStream', J),
                    V)
                        return V(null, J)
                }).catch(function(J) {
                    return q.config.audioFallback && 'DevicesNotFoundError' === J.name && !1 !== H.video ? (H.video = !1,
                    void q.start(H, V)) : (q.emit('localStreamRequestFailed', H),
                    V) ? V(J, null) : void 0
                })
            }
            ,
            ...
            I.prototype.stopStream = function(z) {
                var V = this;
                if (z) {
                    var q = this.localStreams.indexOf(z);
                    -1 < q && (z.getTracks().forEach(function(H) {
                        H.stop()
                    }),
            ...
            I.prototype.startScreenShare = function(z, V) {
                ...
                'function' != typeof z || V || (V = z,
                z = null),
                U(z, function(H, J) {
                    if (H ? q.emit('localScreenRequestFailed') : (q.localScreens.push(J),
                    J.getTracks().forEach(function(W) {
                        W.addEventListener('ended', function() {
                            var X = !0;
            ...
            I.prototype.stopScreenShare = function(z) {
                var V = this;
                if (z) {
                    var q = this.localScreens.indexOf(z);
                    -1 < q && (z.getTracks().forEach(function(H) {
                        H.stop()
                    }),
                    A() && this._removeStream(z))
                } else
                    this.localScreens.forEach(function(H) {
                        H.getTracks().forEach(function(J) {
                            J.stop()
                        }),
                        A() && V._removeStream(H)
            ...
            I.prototype._removeStream = function(z) {
                var V = this.localStreams.indexOf(z);
                -1 < V ? (this.localStreams.splice(V, 1),
                this.emit('localStreamStopped', z)) : (V = this.localScreens.indexOf(z),
                -1 < V && (this.localScreens.splice(V, 1),
                this.emit('localScreenStopped', z)))
            }
            ,
            I.prototype._setupAudioMonitor = function(z, V) {
                this._log('Setup audio');
                var q = N(z, V), H = this, J;
            ...
            I.prototype._stopAudioMonitor = function(z) {
                ...
                -1 < V && (this._audioMonitors[V].audio.stop(),
                this._audioMonitors.splice(V, 1))
            }
            ,
            R.exports = I
        }
        ...
        45: [function(P, R, D) {
            (function(M) {
                ...
                function N(Gr, zr) {
                    for (var Vr = -1, qr = zr.length, Hr = Gr.length; ++Vr < qr; )
                        Gr[Hr + Vr] = zr[Vr];
                    return Gr
                }
                function U(Gr, zr, Vr, qr) {
                    var Hr = -1
                      , Jr = Gr ? Gr.length : 0;
                    for (qr && Jr && (Vr = Gr[++Hr]); ++Hr < Jr; )
                        Vr = zr(Vr, Gr[Hr], Hr, Gr);
                    return Vr
                }
                function F(Gr, zr) {
                    for (var Vr = -1, qr = Array(Gr); ++Vr < Gr; )
                        qr[Vr] = zr(Vr);
                    return qr
                }
                function G(Gr, zr) {
                ...
                function z(Gr) {
                    var zr = !1;
                    if (null != Gr && 'function' != typeof Gr.toString)
                        try {
                            zr = !!(Gr + '')
                        } catch (Vr) {}
                    return zr
                ...
                function Te(Gr, zr, Vr, qr, Hr, Jr, Wr) {
                    var Xr;
                    if (qr && (Xr = Jr ? qr(Gr, Hr, Jr, Wr) : qr(Gr)),
                    void 0 != Xr)
                        return Xr;
                    if (!it(Gr))
                        return Gr;
                ...
                function _e(Gr) {
                    if (!$e(Gr))
                        return Sr(Gr);
                    var zr = [];
                    for (var Vr in Object(Gr))
                        cr.call(Gr, Vr) && 'constructor' != Vr && zr.push(Vr);
                    return zr
                }
                function De(Gr, zr) {
                    if (zr)
                ...
                function qe(Gr) {
                    ...
                    return zr && 'string' == typeof Gr[0] && cr.call(Gr, 'index') && (Vr.index = Gr.index,
                    Vr.input = Gr.input),
                    Vr
                }
                function He(Gr) {
                ...
                function We(Gr, zr) {
                    return zr = null == zr ? ft : zr,
                    !!zr && ('number' == typeof Gr || Wt.test(Gr)) && -1 < Gr && 0 == Gr % 1 && Gr < zr
                }
                function Xe(Gr) {
                    var zr = typeof Gr;
                ...
                Q.prototype['delete'] = function(Gr) {
                    ...
                    if (0 > Vr)
                        return !1;
                    var qr = zr.length - 1;
                    return Vr == qr ? zr.pop() : vr.call(zr, Vr, 1),
                    !0
                }
                ,
                ...
                fe.prototype.set = function(Gr, zr) {
                    ...
                    if (Vr instanceof Q) {
                        var qr = Vr.__data__;
                        if (!Tr || qr.length < 200 - 1)
                            return qr.push([Gr, zr]),
                            this;
        ...
        , {}],
        46: [function(P, R) {
            for (var M = 'assert,count,debug,dir,dirxml,error,exception,group,groupCollapsed,groupEnd,info,log,markTimeline,profile,profileEnd,time,timeEnd,trace,warn'.split(','), A = M.length, I = function() {}, B = {}; A--; )
                B[M[A]] = I;
            R.exports = B
        }
        , {}],
        47: [function(P, R) {
            ...
            R.exports = function(V, q) {
                return q = q || {},
                'string' == typeof V ? M(V) : q.long ? I(V) : A(V)
            }
        }
        , {}],
        48: [function(P, R, D) {
        ...
        49: [function(P, R) {
            (function(M) {
                ...
                R.exports = function(z) {
                    return 'string' == typeof z && z ? (z = z.replace(U, '').replace(F, ''),
                    M.JSON && JSON.parse ? JSON.parse(z) : A.test(z.replace(I, '@').replace(B, ']').replace(N, '')) ? new Function('return ' + z)() : void 0) : null
                }
            }
            ).call(this, 'undefined' == typeof global ? 'undefined' == typeof self ? 'undefined' == typeof window ? {} : window : self : global)
        }
        ...
        50: [function(P, R, D) {
            D.encode = function(M) {
                var A = '';
                for (var I in M)
                    M.hasOwnProperty(I) && (A.length && (A += '&'),
                    A += encodeURIComponent(I) + '=' + encodeURIComponent(M[I]));
                return A
            }
            ,
            D.decode = function(M) {
                for (var A = {}, I = M.split('&'), B = 0, N = I.length, U; B < N; B++)
                    U = I[B].split('='),
        ...
        53: [function(P, R) {
            function M(U, F) {
                var G = this, z;
                for (z in B.call(this),
                U = U || {},
                U.iceServers = U.iceServers || [],
                this.enableChromeNativeSimulcast = !1,
                F && F.optional && window.chrome && null === navigator.appVersion.match(/Chromium\//) && F.optional.forEach(function(V) {
                    V.enableChromeNativeSimulcast && (G.enableChromeNativeSimulcast = !0)
                }),
                this.enableMultiStreamHacks = !1,
                F && F.optional && window.chrome && F.optional.forEach(function(V) {
                ...
                window.navigator.mozGetUserMedia && F && F.optional && (this.wtFirefox = 0,
                F.optional.forEach(function(V) {
                    V.andyetFirefoxMakesMeSad && (G.wtFirefox = V.andyetFirefoxMakesMeSad,
                    0 < G.wtFirefox && (G.firefoxcandidatebuffer = []))
                })),
                this.pc = new RTCPeerConnection(U,F),
                this.getLocalStreams = 'function' == typeof this.pc.getLocalStreams ? this.pc.getLocalStreams.bind(this.pc) : function() {
                    return []
                }
                ,
                this.getRemoteStreams = this.pc.getRemoteStreams.bind(this.pc),
                this.addStream = this.pc.addStream.bind(this.pc),
                this.removeStream = function(V) {
                ...
                ,
                'function' == typeof this.pc.removeTrack && (this.removeTrack = this.pc.removeTrack.bind(this.pc)),
            ...
            M.prototype._checkLocalCandidate = function(U) {
                ...
                'srflx' == F.type ? this.hadLocalStunCandidate = !0 : 'relay' == F.type && (this.hadLocalRelayCandidate = !0),
                -1 != F.ip.indexOf(':') && (this.hadLocalIPv6Candidate = !0)
            }
            ,
            M.prototype._checkRemoteCandidate = function(U) {
                var F = I.toCandidateJSON(U);
                'srflx' == F.type ? this.hadRemoteStunCandidate = !0 : 'relay' == F.type && (this.hadRemoteRelayCandidate = !0),
                -1 != F.ip.indexOf(':') && (this.hadRemoteIPv6Candidate = !0)
            }
            ,
            M.prototype.processIce = function(U, F) {
                ...
                if (U.contents || U.jingle && U.jingle.contents) {
                    ...
                    V.forEach(function(q) {
                        ...
                        if (!(G.iceCredentials.remote[q.name] && H.ufrag && G.iceCredentials.remote[q.name].ufrag !== H.ufrag))
                            K();
                        else if (Y) {
                            ...
                            G.emit('error', 'ice restart failed to find matching content')
                    })
                } else {
                    if (U.candidate && 0 !== U.candidate.candidate.indexOf('a=') && (U.candidate.candidate = 'a=' + U.candidate.candidate),
                    this.wtFirefox && null !== this.firefoxcandidatebuffer && this.pc.localDescription && 'offer' === this.pc.localDescription.type)
                        return this.firefoxcandidatebuffer.push(U.candidate),
                        F();
            ...
            M.prototype.offer = function(U, F) {
                ...
                F = F || function() {}
                ,
                'closed' === this.pc.signalingState ? F('Already closed') : void this.pc.createOffer(function(q) {
                    var H = {
                        type: 'offer',
                        sdp: q.sdp
                    };
                    G.assumeSetLocalSuccess && (G.emit('offer', H),
                    F(null, H)),
                    G._candidateBuffer = [],
            ...
            M.prototype.handleAnswer = function(U, F) {
                ...
                G.remoteDescription = U.jingle,
                U.jingle.contents.forEach(function(z) {
                    var V = z.transport || {};
            ...
            M.prototype._onIce = function(U) {
                ...
                if (U.candidate) {
                    ...
                    var V = I.toCandidateJSON(G.candidate), q, H;
                    ...
                    H = q.indexOf(V.foundation + ':' + V.component),
                    -1 < H && V.priority >> 24 >= q[H].priority >> 24))
                        return;
                    ...
                    H = q.indexOf(V.address + ':' + V.port),
                    -1 < H))
                        return;
                    if ('require' === this.config.rtcpMuxPolicy && '2' === V.component)
                        return;
                    if (this._candidateBuffer.push(V),
                    F.config.useJingle) {
                        if (G.sdpMid || (F.pc.remoteDescription && 'offer' === F.pc.remoteDescription.type ? G.sdpMid = F.remoteDescription.contents[G.sdpMLineIndex].name : G.sdpMid = F.localDescription.contents[G.sdpMLineIndex].name),
                        !F.iceCredentials.local[G.sdpMid]) {
                        ...
                        0 < F.batchIceCandidates)
                            return 0 === F.batchedIceCandidates.length && window.setTimeout(function() {
                                var W = {};
        ...
        55: [function(P, R, D) {
            ...
            D.findLines = function(M, A, I) {
                for (var B = [], N = M.length, U = 0; U < A.length; U++)
                    A[U].substr(0, N) === M && B.push(A[U]);
                if (B.length || !I)
                    return B;
                for (var F = 0; F < I.length; F++)
                    I[F].substr(0, N) === M && B.push(I[F]);
                return B
            }
            ,
            D.mline = function(M) {
            ...
            ,
            D.fmtp = function(M) {
                for (var N = M.substr(M.indexOf(' ') + 1).split(';'), U = [], F = 0, A, I, B; F < N.length; F++)
                    A = N[F].split('='),
                    I = A[0].trim(),
                    B = A[1],
                    I && B ? U.push({
            ...
            D.extmap = function(M) {
                var A = M.substr(9).split(' ')
                  ...
                  , B = A.shift()
                  , N = B.indexOf('/');
                return 0 <= N ? (I.id = B.substr(0, N),
                I.senders = B.substr(N + 1)) : (I.id = B,
                I.senders = 'sendrecv'),
                I.uri = A.shift() || '',
                I
            }
            ...
            D.rtcpfb = function(M) {
                ...
                return I.id = A.shift(),
                I.type = A.shift(),
                'trr-int' === I.type ? I.value = A.shift() : I.subtype = A.shift() || '',
                I.parameters = A,
                I
            }
            ,
            D.candidate = function(M) {
                ...
                for (var I = {
                    ...
                    'raddr' === A[B] ? I.relAddr = A[B + 1] : 'rport' === A[B] ? I.relPort = A[B + 1] : 'generation' === A[B] ? I.generation = A[B + 1] : 'tcptype' === A[B] && (I.tcpType = A[B + 1]);
                return I.network = '1',
                I
            }
            ,
            D.sourceGroups = function(M) {
                for (var A = [], I = 0, B; I < M.length; I++)
                    B = M[I].substr(13).split(' '),
                    A.push({
                        semantics: B.shift(),
                        sources: B
                    });
            ...
            ,
            D.groups = function(M) {
                for (var A = [], B = 0, I; B < M.length; B++)
                    I = M[B].substr(8).split(' '),
                    A.push({
                        semantics: I.shift(),
                        contents: I
                    });
        ...
        57: [function(P, R, D) {
            ...
            D.toSessionJSON = function(B, N) {
                var F = N.creators || [], G = N.role || 'initiator', z = N.direction || 'outgoing', V = B.split('\r\nm='), U;
                for (U = 1; U < V.length; U++)
                    V[U] = 'm=' + V[U],
                    U !== V.length - 1 && (V[U] += '\r\n');
                var q = V.shift() + '\r\n'
                  , H = A.lines(q)
            ...
            D.toMediaJSON = function(B, N, U) {
                ...
                if (Y && (J.name = Y.substr(6)),
                A.findLine('a=sendrecv', V, q) ? J.senders = 'both' : A.findLine('a=sendonly', V, q) ? J.senders = M[G][z].sendonly : A.findLine('a=recvonly', V, q) ? J.senders = M[G][z].recvonly : A.findLine('a=inactive', V, q) && (J.senders = 'none'),
                'rtp' == W.applicationType) {
                    ...
                    if (ce) {
                        ...
                        ['msid', 'mslabel', 'label'].forEach(function(ye) {
                            for (var be = 0, ve; be < de.length; be++) {
                                ve = !1;
                                for (var ke = 0; ke < de[be].parameters.length; ke++)
                                    de[be].parameters[ke].key === ye && (ve = !0);
        ...
        58: [function(P, R, D) {
            ...
            D.toMediaSDP = function(A, I) {
                var B = []
                  ...
                  , V = G && G.fingerprints || []
                  , q = [];
                if ('datachannel' == F.applicationType ? (q.push('application'),
                q.push('1'),
                q.push('DTLS/SCTP'),
                ...
                q.push('1'),
                0 < V.length ? q.push('UDP/TLS/RTP/SAVPF') : F.encryption && 0 < F.encryption.length ? q.push('RTP/SAVPF') : q.push('RTP/AVPF'),
                z.forEach(function(Z) {
                    q.push(Z.id)
                })),
                B.push('m=' + q.join(' ')),
                B.push('c=IN IP4 0.0.0.0'),
                F.bandwidth && F.bandwidth.type && F.bandwidth.bandwidth && B.push('b=' + F.bandwidth.type + ':' + F.bandwidth.bandwidth),
                'rtp' == F.applicationType && B.push('a=rtcp:1 IN IP4 0.0.0.0'),
                G) {
                ...
                z.forEach(function(Z) {
                    ...
                    if (Z.channels && '1' != Z.channels && (Q += '/' + Z.channels),
                    B.push(Q),
                    Z.parameters && Z.parameters.length) {
                        var ne = ['a=fmtp:' + Z.id]
                          , ae = [];
                ...
                Y.forEach(function(Z) {
                    for (var Q = 0, ne; Q < Z.parameters.length; Q++)
                        ne = Z.parameters[Q],
            ...
            D.toCandidateSDP = function(A) {
                ...
                return I.push('typ'),
                I.push(B),
                ('srflx' === B || 'prflx' === B || 'relay' === B) && A.relAddr && A.relPort && (I.push('raddr'),
                I.push(A.relAddr),
                I.push('rport'),
                I.push(A.relPort)),
                A.tcpType && 'TCP' == A.protocol.toUpperCase() && (I.push('tcptype'),
                I.push(A.tcpType)),
                I.push('generation'),
                I.push(A.generation || '0'),
                'a=candidate:' + I.join(' ')
            }
        }
        , {
        ...
        61: [function(P, R, D) {
            function M(F, G) {
                ...
                F = void 0),
                G = G || {};
                var z = A(F), V = z.source, q = z.id, H;
                return G.forceNew || G['force new connection'] || !1 === G.multiplex ? (N('ignoring socket cache for %s', V),
                H = B(V, G)) : (!U[q] && (N('new io instance for %s', V),
                U[q] = B(V, G)),
                H = U[q]),
                H.socket(z.path)
            }
        ...
        62: [function(P, R) {
            function M(J, W) {
                return this instanceof M ? void (J && 'object' == typeof J && (W = J,
                J = void 0),
                W = W || {},
                W.path = W.path || '/socket.io',
                this.nsps = {},
                this.subs = [],
                this.opts = W,
                this.reconnection(!1 !== W.reconnection),
                this.reconnectionAttempts(W.reconnectionAttempts || Infinity),
                this.reconnectionDelay(W.reconnectionDelay || 1e3),
                this.reconnectionDelayMax(W.reconnectionDelayMax || 5e3),
                this.randomizationFactor(W.randomizationFactor || 0.5),
                this.backoff = new H({
                    min: this.reconnectionDelay(),
                    max: this.reconnectionDelayMax(),
                    jitter: this.randomizationFactor()
                }),
                this.timeout(null == W.timeout ? 2e4 : W.timeout),
                this.readyState = 'closed',
                this.uri = J,
                this.connected = [],
                this.encoding = !1,
                ...
                this.autoConnect = !1 !== W.autoConnect,
                this.autoConnect && this.open()) : new M(J,W)
            }
            var A = P('./url')
            ...
            M.prototype.emitAll = function() {
                ...
                this.nsps)
                    this.nsps[J].emit.apply(this.nsps[J], arguments)
            }
            ,
            M.prototype.updateSocketIds = function() {
                for (var J in this.nsps)
                    this.nsps[J].id = this.engine.id
            }
            ,
            N(M.prototype),
            M.prototype.reconnection = function(J) {
                return arguments.length ? (this._reconnection = !!J,
                this) : this._reconnection
            }
            ,
            M.prototype.reconnectionAttempts = function(J) {
                return arguments.length ? (this._reconnectionAttempts = J,
                this) : this._reconnectionAttempts
            }
            ,
            M.prototype.reconnectionDelay = function(J) {
                return arguments.length ? (this._reconnectionDelay = J,
                this.backoff && this.backoff.setMin(J),
                this) : this._reconnectionDelay
            }
            ,
            M.prototype.randomizationFactor = function(J) {
                return arguments.length ? (this._randomizationFactor = J,
                this.backoff && this.backoff.setJitter(J),
                this) : this._randomizationFactor
            }
            ,
            M.prototype.reconnectionDelayMax = function(J) {
                return arguments.length ? (this._reconnectionDelayMax = J,
                this.backoff && this.backoff.setMax(J),
                this) : this._reconnectionDelayMax
            }
            ,
            M.prototype.timeout = function(J) {
                return arguments.length ? (this._timeout = J,
                this) : this._timeout
            }
            ,
            ...
            M.prototype.open = M.prototype.connect = function(J) {
                if (V('readyState %s', this.readyState),
                ...
                var Y = F(W, 'open', function() {
                  ...
                  , K = F(W, 'error', function(ne) {
                    ...
                    X.cleanup(),
                    X.readyState = 'closed',
                    X.emitAll('connect_error', ne),
                    J) {
                        var ae = new Error('Connection error');
                        ae.data = ne,
                        J(ae)
                    } else
                        X.maybeReconnectOnOpen()
                });
                if (!1 !== this._timeout) {
                    var Z = this._timeout;
            ...
            M.prototype.socket = function(J) {
                ...
                if (!W) {
                    ...
                    W.on('connect', function() {
                        W.id = X.engine.id,
                        ~q(X.connected, W) || X.connected.push(W)
                    })
                }
                return W
            }
            ...
            M.prototype.packet = function(J) {
                ...
                var W = this;
                W.encoding ? W.packetBuffer.push(J) : (W.encoding = !0,
                this.encoder.encode(J, function(X) {
            ...
            M.prototype.reconnect = function() {
                ...
                if (this.backoff.attempts >= this._reconnectionAttempts)
                    ...
                    var X = setTimeout(function() {
                        J.skipReconnect || (V('attempting reconnect'),
                        J.emitAll('reconnect_attempt', J.backoff.attempts),
                        J.emitAll('reconnecting', J.backoff.attempts),
                        J.skipReconnect || J.open(function(Y) {
                            Y ? (V('reconnect attempt error'),
                            J.reconnecting = !1,
                            J.reconnect(),
                            J.emitAll('reconnect_error', Y.data)) : (V('reconnect success'),
                            J.onreconnect())
                        }))
                    }, W);
                    this.subs.push({
        ...
        64: [function(P, R, D) {
            ...
            M.prototype.emit = function(q) {
                ...
                return 'function' == typeof H[H.length - 1] && (F('emitting packet with ack id %d', this.ids),
                this.acks[this.ids] = H.pop(),
                W.id = this.ids++),
            ...
            M.prototype.onevent = function(q) {
                ...
                null != q.id && (F('attaching ack callback to event'),
                H.push(this.ack(q.id))),
            ...
            M.prototype.emitBuffered = function() {
                ...
                for (q = 0; q < this.receiveBuffer.length; q++)
                    V.apply(this, this.receiveBuffer[q]);
                for (this.receiveBuffer = [],
                q = 0; q < this.sendBuffer.length; q++)
            ...
            ,
            M.prototype.close = M.prototype.disconnect = function() {
                return this.connected && (F('performing disconnect (%s)', this.nsp),
                this.packet({
                    type: A.DISCONNECT
                })),
                this.destroy(),
        ...
        65: [function(P, R) {
            (function(M) {
                ...
                R.exports = function(N, U) {
                    var F = N
                      , U = U || M.location;
                    return null == N && (N = U.protocol + '//' + U.host),
                    'string' == typeof N && ('/' == N.charAt(0) && ('/' == N.charAt(1) ? N = U.protocol + N : N = U.hostname + N),
                    !/^(https?|wss?):\/\//.test(N) && (B('protocol-less url %s', N),
                    N = 'undefined' == typeof U ? 'https://' + N : U.protocol + '//' + N),
                    B('parse %s', N),
                    F = I(N)),
            ...
            (function(M) {
                ...
                D.deconstructPacket = function(B) {
                    function N(z) {
                        ...
                        if (A(z)) {
                            for (var q = Array(z.length), H = 0; H < z.length; H++)
                                q[H] = N(z[H]);
                            return q
                        }
        ...
        67: [function(P, R, D) {
            ...
            function A(W) {
                var X = ''
                  , Y = !1;
                return X += W.type,
                (D.BINARY_EVENT == W.type || D.BINARY_ACK == W.type) && (X += W.attachments,
                X += '-'),
                W.nsp && '/' != W.nsp && (Y = !0,
                X += W.nsp),
                null != W.id && (Y && (X += ',',
            ...
            function N(W) {
                var X = {}
                  , Y = 0;
                if (X.type = +W.charAt(0),
                null == D.types[X.type])
                    return F();
                if (D.BINARY_EVENT == X.type || D.BINARY_ACK == X.type) {
                    for (var K = ''; '-' != W.charAt(++Y) && (K += W.charAt(Y),
                    Y != W.length); )
                        ;
                    if (K != +K || '-' != W.charAt(Y))
            ...
            M.prototype.encode = function(W, X) {
                if (G('encoding packet %j', W),
                D.BINARY_EVENT == W.type || D.BINARY_ACK == W.type)
                    I(W, X);
                else {
                    var Y = A(W);
                    X([Y])
            ...
            B.prototype.add = function(W) {
                ...
                if ('string' == typeof W)
                    X = N(W),
                    D.BINARY_EVENT == X.type || D.BINARY_ACK == X.type ? (this.reconstructor = new U(X),
                    0 === this.reconstructor.reconPack.attachments && this.emit('decoded', X)) : this.emit('decoded', X);
                else if (!(J(W) || W.base64))
                    throw new Error('Unknown type: ' + W);
                else if (!this.reconstructor)
                    throw new Error('got binary data when not reconstructing a packet');
                else
                    X = this.reconstructor.takeBinaryData(W),
                    X && (this.reconstructor = null,
                    this.emit('decoded', X))
            }
            ,
        ...
        70: [function(P, R, D) {
            (function(M) {
                (function(A) {
                    function I(ie) {
                        for (var se = [], de = 0, ce = ie.length, pe, le; de < ce; )
                            pe = ie.charCodeAt(de++),
                            55296 <= pe && 56319 >= pe && de < ce ? (le = ie.charCodeAt(de++),
                            56320 == (64512 & le) ? se.push(((1023 & pe) << 10) + (1023 & le) + 65536) : (se.push(pe),
                            de--)) : se.push(pe);
                        return se
                    }
                    function B(ie) {
                        for (var se = ie.length, de = -1, pe = '', ce; ++de < se; )
                            ce = ie[de],
                            65535 < ce && (ce -= 65536,
                            pe += X(55296 | 1023 & ce >>> 10),
                            ce = 56320 | 1023 & ce),
                            pe += X(ce);
                        return pe
                    }
                    function N(ie) {
                        if (55296 <= ie && 57343 >= ie)
                            throw Error('Lone surrogate U+' + ie.toString(16).toUpperCase() + ' is not a scalar value')
                    }
                    function U(ie, se) {
                    ...
                    function F(ie) {
                        if (0 == (4294967168 & ie))
                            return X(ie);
                        var se = '';
                        return 0 == (4294965248 & ie) ? se = X(192 | 31 & ie >> 6) : 0 == (4294901760 & ie) ? (N(ie),
                        se = X(224 | 15 & ie >> 12),
                        se += U(ie, 6)) : 0 == (4292870144 & ie) && (se = X(240 | 7 & ie >> 18),
                        se += U(ie, 12),
                        se += U(ie, 6)),
                        se += X(128 | 63 & ie),
                        se
                    ...
                    var X = String.fromCharCode, Y, K, Z, Q = {
                        version: '2.0.0',
                        encode: function(ie) {
                            for (var se = I(ie), de = se.length, ce = -1, le = '', pe; ++ce < de; )
                                pe = se[ce],
                                le += F(pe);
                            return le
                        },
                        decode: function(ie) {
                            ...
                            K = Y.length,
                            Z = 0;
                            for (var se = [], de; !1 !== (de = V()); )
                                se.push(de);
                            return B(se)
                        }
                    };
                    if ('function' == typeof C && 'object' == typeof C.amd && C.amd)
                        C(function() {
                            return Q
                        });
            ...
            (function(M, A) {
                ...
                function F(be, ve, ke) {
                    ...
                    var xe = Object.keys(ve)
                      , Te = U(xe);
                    if (be.showHidden && (xe = Object.getOwnPropertyNames(ve)),
                    se(ve) && (0 <= xe.indexOf('message') || 0 <= xe.indexOf('description')))
                        return z(ve);
                    if (0 === xe.length) {
                        if (de(ve)) {
                        ...
                        if (ae(ve))
                            return be.stylize(RegExp.prototype.toString.call(ve), 'regexp');
                        if (ie(ve))
                            return be.stylize(Date.prototype.toString.call(ve), 'date');
                        if (se(ve))
                            return z(ve)
                    }
                    var Ee = ''
                    ...
                    if (ae(ve) && (Ee = ' ' + RegExp.prototype.toString.call(ve)),
                    ie(ve) && (Ee = ' ' + Date.prototype.toUTCString.call(ve)),
                    se(ve) && (Ee = ' ' + z(ve)),
                    0 === xe.length && (!Pe || 0 == ve.length))
                        return Re[0] + Ee + Re[1];
                    if (0 > ke)
                        return ae(ve) ? be.stylize(RegExp.prototype.toString.call(ve), 'regexp') : be.stylize('[Object]', 'special');
                    be.seen.push(ve);
                    var De;
                ...
                function G(be, ve) {
                    if (ne(ve))
                        return be.stylize('undefined', 'undefined');
                    if (Z(ve)) {
                ...
                function V(be, ve, ke, Ce, Se) {
                    for (var xe = [], Te = 0, we = ve.length; Te < we; ++Te)
                        me(ve, Te + '') ? xe.push(q(be, ve, ke, Ce, Te + '', !0)) : xe.push('');
                ...
                function q(be, ve, ke, Ce, Se, xe) {
                    ...
                    Ee.get ? Ee.set ? we = be.stylize('[Getter/Setter]', 'special') : we = be.stylize('[Getter]', 'special') : Ee.set && (we = be.stylize('[Setter]', 'special')),
                    me(Ce, Se) || (Te = '[' + Se + ']'),
                    we || (0 > be.seen.indexOf(Ee.value) ? (we = X(ke) ? F(be, Ee.value, null) : F(be, Ee.value, ke - 1),
                    -1 < we.indexOf('\n') && (xe ? we = we.split('\n').map(function(Pe) {
                        return '  ' + Pe
                    }).join('\n').substr(2) : we = '\n' + we.split('\n').map(function(Pe) {
                        return '   ' + Pe
                    }).join('\n'))) : we = be.stylize('[Circular]', 'special')),
                    ne(Te)) {
                        if (xe && Se.match(/^\d+$/))
                            return we;
                        Te = JSON.stringify('' + Se),
                        Te.match(/^"([a-zA-Z_][a-zA-Z_0-9]*)"$/) ? (Te = Te.substr(1, Te.length - 2),
                        Te = be.stylize(Te, 'name')) : (Te = Te.replace(/'/g, '\\\'').replace(/\\"/g, '"').replace(/(^"|"$)/g, '\''),
                        Te = be.stylize(Te, 'string'))
                    }
                    return Te + ': ' + we
                }
                function H(be, ve, ke) {
                ...
                D.format = function(be) {
                    if (!Z(be)) {
                        for (var ve = [], ke = 0; ke < arguments.length; ke++)
                            ve.push(I(arguments[ke]));
                        return ve.join(' ')
                    }
                    ...
                    }), Te = Ce[ke]; ke < Se; Te = Ce[++ke])
                        xe += X(Te) || !oe(Te) ? ' ' + Te : ' ' + I(Te);
                    return xe
                }
                ,
                    ...
                    return function() {
                        if (!Ce) {
                            if (M.throwDeprecation)
                                ...
                                M.traceDeprecation ? console.trace(ve) : console.error(ve);
                            Ce = !0
                        }
                ...
                D._extend = function(be, ve) {
                    if (!ve || !oe(ve))
                        return be;
                    for (var ke = Object.keys(ve), Ce = ke.length; Ce--; )
                        be[ke[Ce]] = ve[ke[Ce]];
                    return be
                }
            }
        ...
        74: [function(P, R) {
            var M, A;
            window.mozRTCPeerConnection || navigator.mozGetUserMedia ? (M = 'moz',
            A = parseInt(navigator.userAgent.match(/Firefox\/([0-9]+)\./)[1], 10)) : (window.webkitRTCPeerConnection || navigator.webkitGetUserMedia) && (M = 'webkit',
        ...
        75: [function(P, R) {
            ...
            M.mixin = function(A) {
                ...
                I.on = function(B) {
                    ...
                    return z._groupName = G,
                    (this.callbacks[B] = this.callbacks[B] || []).push(z),
                    this
                }
                ,
                I.once = function(B) {
                ...
                I.releaseGroup = function(B) {
                    ...
                    for (N in this.callbacks)
                        for (G = this.callbacks[N],
                        U = 0,
                        F = G.length; U < F; U++)
                            G[U]._groupName === B && (G.splice(U, 1),
                            U--,
                            F--);
                    return this
                }
                ...
                I.off = function(B, N) {
                    this.callbacks = this.callbacks || {};
                    var U = this.callbacks[B], F;
                    return U ? 1 === arguments.length ? (delete this.callbacks[B],
                    this) : (F = U.indexOf(N),
                    U.splice(F, 1),
                    0 === U.length && delete this.callbacks[B],
                    this) : this
                }
                ,
                I.emit = function(B) {
                    this.callbacks = this.callbacks || {};
                    var N = [].slice.call(arguments, 1), U = this.callbacks[B], F = this.getWildcardCallbacks(B), G, z, q;
                    if (U)
                        for (q = U.slice(),
                        G = 0,
                        z = q.length; G < z && !!q[G]; ++G)
                            q[G].apply(this, N);
                    if (F)
                        for (z = F.length,
                        q = F.slice(),
                ...
                I.getWildcardCallbacks = function(B) {
                    ...
                    for (N in this.callbacks)
                        U = N.split('*'),
                        ('*' === N || 2 === U.length && B.slice(0, U[0].length) === U[0]) && (F = F.concat(this.callbacks[N]));
                    return F
                }
            }
            ,
        ...
        77: [function(P, R) {
            ...
            function A(z) {
                ...
                this.pc.on('iceConnectionStateChange', function() {
                    ...
                    case 'failed':
                        'offer' === V.pc.pc.localDescription.type && (V.parent.emit('iceFailed', V),
                        V.send('connectivityError'));
                    }
                }),
                this.pc.on('signalingStateChange', this.emit.bind(this, 'signalingStateChange')),
                this.logger = this.parent.logger,
                'screen' === z.type ? this.parent.localScreens && this.parent.localScreens[0] && this.sharemyscreen && (this.logger.log('adding local screen stream to peer connection'),
                this.pc.addStream(this.parent.localScreens[0]),
                this.broadcaster = z.broadcaster) : this.parent.localStreams.forEach(function(q) {
                    V.pc.addStream(q)
                }),
            ...
            A.prototype.handleMessage = function(z) {
                var V = this;
                if (this.logger.log('getting', z.type, z),
                z.prefix && (this.browserPrefix = z.prefix),
                'offer' === z.type)
                    this.nick || (this.nick = z.payload.nick),
                ...
                else if ('candidate' === z.type)
                    this.pc.processIce(z.payload);
                else if ('connectivityError' === z.type)
                    this.parent.emit('connectivityError', V);
                else if ('mute' === z.type)
                    this.parent.emit('mute', {
                        id: z.from,
                        name: z.payload.name
                    });
                else if ('unmute' === z.type)
            ...
            A.prototype.sendDirectly = function(z, V, q) {
                ...
                return !('open' != J.readyState) && (J.send(JSON.stringify(H)),
                !0)
            }
            ,
            ...
            A.prototype.getDataChannel = function(z, V) {
                if (!B.supportDataChannel)
                    return this.emit('error', new Error('createDataChannel not supported'));
                var q = this.channels[z];
                return (V || (V = {}),
                q) ? q : (q = this.channels[z] = this.pc.createDataChannel(z, V),
                this._observeDataChannel(q),
                q)
            }
            ,
            ...
            A.prototype.handleRemoteStreamAdded = function(z) {
                var V = this;
                this.stream ? this.logger.warn('Already have a remote stream') : (this.stream = z.stream,
                this.stream.getTracks().forEach(function(q) {
            ...
            A.prototype.handleStreamRemoved = function() {
                var z = this.parent.peers.indexOf(this);
                -1 < z && (this.parent.peers.splice(z, 1),
                this.closed = !0,
        ...
        78: [function(P, R) {
            function M(G) {
                ...
                V)
                    V.hasOwnProperty(H) && (this.config[H] = V[H]);
                this.capabilities = B,
                I.call(this),
                ...
                J.on('message', function(W) {
                    var X = z.webrtc.getPeers(W.from, W.roomType), Y;
                    ...
                    z.emit('createdPeer', Y)),
                    Y.handleMessage(W)) : X.length && X.forEach(function(K) {
                ...
                G.debug = !1,
                this.webrtc = new A(G),
                ['mute', 'unmute', 'pauseVideo', 'resumeVideo', 'pause', 'resume', 'sendToAll', 'sendDirectlyToAll', 'getPeers'].forEach(function(W) {
            ...
            M.prototype.joinRoom = function(G, z) {
                ...
                this.connection.emit('join', G, function(q, H) {
                    if (console.log('join CB', q, H),
                    q)
                        V.emit('error', q);
                    else {
                        var J, W, X, Y;
                        for (J in H.clients)
                            for (X in W = H.clients[J],
                            W)
                                W[X] && (Y = V.webrtc.createPeer({
                                    id: J,
                                    type: X,
            ...
            M.prototype.getLocalVideoContainer = function() {
                ...
                if (G && 'VIDEO' === G.tagName)
                    return G.oncontextmenu = function() {
                        return !1
                    }
                    ,
                    G;
